...
class TestFlaskAppIntegration:
    ...
    @patch(
        "src.services.report_data_service.ReportDataService.get_monthly_revenue_report_data"
    )
    ...
    def test_request_parameter_extraction(self):
        ...
        response = self.client.get(
            "/api/revenue/monthly/2025?customer_search=test&ae_filter=John"
        )
    ...
    def test_template_context_preparation(self):
        ...
        with patch(
            ...
            assert response.status_code == 200
            assert (
                b"Monthly Revenue Dashboard" in response.data
                or b"html" in response.data.lower()
            )
...
class TestPerformanceIntegration:
    ...
    def test_service_caching(self):
        ...
        start_time = time.time()
        years1 = report_service._get_available_years(
            container.get("database_connection")
        )
        first_call_time = time.time() - start_time

        start_time = time.time()
        years2 = report_service._get_available_years(
            container.get("database_connection")
        )
