...
def create_test_database():
    ...
    # Test spots
    cursor.execute("INSERT INTO spots VALUES (1, 'E', 'Standard', 1, 100.0, 'TEST001')")  # English
    cursor.execute("INSERT INTO spots VALUES (2, 'L', 'Standard', 1, 500.0, 'TEST002')")  # Undetermined
    cursor.execute("INSERT INTO spots VALUES (3, NULL, 'Standard', 1, 50.0, 'TEST003')")  # No language
    cursor.execute("INSERT INTO spots VALUES (4, 'X', 'Standard', 1, 200.0, 'TEST004')")  # Invalid
    cursor.execute("INSERT INTO spots VALUES (5, 'M', 'Standard', 1, 1000.0, 'TEST005')")  # Mandarin
...
def test_language_assignment():
    ...
    for spot_id, expected_status, expected_review, description in test_cases:
        try:
            ...
            if status_match and review_match:
                print(f"✅ PASS: {description} - Status: {assignment.language_status.value}, Review: {assignment.requires_review}")
                passed += 1
            else:
                print(f"❌ FAIL: {description} - Expected: {expected_status.value}/{expected_review}, Got: {assignment.language_status.value}/{assignment.requires_review}")
                failed += 1
    ...
    if len(batch_results) == 5:
        ...
        print(f"❌ FAIL: Batch processing returned {len(batch_results)} results, expected 5")
