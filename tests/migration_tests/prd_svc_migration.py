...
def analyze_prd_svc_patterns(year="2024", db_connection=None):
    ...
    return {"prd_breakdown": prd_breakdown, "svc_breakdown": svc_breakdown}
...
def test_prd_svc_migration(year="2024"):
    ...
    try:
        ...
        print(
            f"   Average Rate: ${prd_result.revenue / prd_result.spot_count:,.2f}"
            if prd_result.spot_count > 0
        ...
        print(
            f"   Average Rate: ${svc_result.revenue / svc_result.spot_count:,.2f}"
            if svc_result.spot_count > 0
            else "   Average Rate: N/A"
        )
        ...
        if patterns["prd_breakdown"]:
            for category, customer, bill_code, spots, revenue in patterns[
                ...
                print(
                    f"   {customer:<30} | {bill_code:<20}: ${revenue:>10,.2f} ({spots:>3,} spots)"
                )
        else:
        ...
        print(
            f"   • Combined: ${prd_result.revenue + svc_result.revenue:,.2f} ({(prd_result.revenue + svc_result.revenue) / 4076255.94 * 100:.1f}%)"
        )
