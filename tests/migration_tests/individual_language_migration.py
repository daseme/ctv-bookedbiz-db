...
import sqlite3
from query_builders import IndividualLanguageQueryBuilder, BaseQueryBuilder, validate_query_migration
...
def test_individual_language_migration(year="2024"):
    ...
    try:
        ...
        for lang_data in detailed_results['languages']:
            ...
            print(f"   {lang_data['language']:<15} ${lang_data['revenue']:>12,.2f} "
                  f"{lang_data['spots']:>6,} {lang_data['bonus_spots']:>6,} {pct:>6.1f}%")
        
        print("   " + "-" * 60)
        print(f"   {'TOTAL':<15} ${detailed_results['total_revenue']:>12,.2f} "
        ...
        difference = abs(detailed_results['total_revenue'] - documented_individual_total)
        ...
        top_languages = sorted(detailed_results['languages'], 
...
def show_chinese_combination_effect(year="2024"):
    ...
    try:
        ...
        chinese_from_full = next((lang['revenue'] for lang in full_results['languages'] 
