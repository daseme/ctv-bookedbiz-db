...
try:
    from services.language_block_service import (
        LanguageBlockService,
        test_language_block_service,
    )
...
class BasicLanguageBlockTest:
    ...
    def test_database_schema(self):
        ...
        required_tables = [
            ...
            "markets",
            "languages",
            "programming_schedules",
            "schedule_market_assignments",
            "language_blocks",
    ...
    def test_spot_data_quality(self):
        ...
        print(
            f"📊 Assignable spots: {assignable_spots:,} ({assignable_spots / total_spots * 100:.1f}%)"
        )
        ...
        if unassigned_spots == 0:
            print(
                "⚠️  No unassigned spots found - may need to clear existing assignments for testing"
            )
            return False
    ...
    def test_single_spot_assignment(self):
        ...
        print(
            f"🎯 Testing spot {spot_id} ({bill_code}) - {market_code} {air_date} {time_in}-{time_out}"
        )
        ...
        try:
            ...
            if result.success:
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM spot_language_blocks WHERE spot_id = ?",
                    (spot_id,),
                )
        ...
        try:
            ...
            if results["spot_details"]:
                ...
                for detail in results["spot_details"][:5]:  # Show first 5
                    ...
                    print(
                        f"   Spot {detail['spot_id']} ({detail['bill_code']}): {detail['customer_intent']} -> {block_info}{attention}"
                    )
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            if failed == 0:
                print(
                    "✅ ALL TESTS PASSED - Language Block Service is ready for Phase 2!"
                )
                return True
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Basic Language Block Service Integration Test"
    )
    parser.add_argument("--db-path", required=True, help="Path to database file")
    parser.add_argument(
        "--limit", type=int, default=10, help="Number of spots for batch test"
    )
