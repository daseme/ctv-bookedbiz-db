...
def test_pipeline_decay_system():
    ...
    try:
        ...
        if success:
            ...
            if decay_summary:
                print(f"✅ Calibrated pipeline: {decay_summary['calibrated_pipeline']:,}")
        ...
        if success:
            ...
            if decay_summary:
                ...
                print(f"✅ Pipeline after booking: {actual_pipeline:,} (expected: {expected_pipeline:,})")
                ...
                if actual_pipeline == expected_pipeline:
                    ...
                    print(f"❌ Decay calculation incorrect: {actual_pipeline} != {expected_pipeline}")
                    return False
            else:
        ...
        if success:
            ...
            decay_summary = pipeline_service.get_pipeline_decay_summary(ae_id, month)
            if decay_summary:
                expected_pipeline = initial_pipeline - booking_amount + removal_amount  # 50000 - 8000 + 3000 = 45000
                actual_pipeline = decay_summary['current_pipeline']
                
                print(f"✅ Pipeline after removal: {actual_pipeline:,} (expected: {expected_pipeline:,})")
                print(f"✅ Total decay: {decay_summary['total_decay']:+,}")
                print(f"✅ Decay events: {len(decay_summary['decay_events'])}")
        ...
        if decay_summary:
            ...
            if actual_final == expected_final:
                ...
                print(f"❌ Multiple changes calculation incorrect: {actual_final} != {expected_final}")
                return False
        ...
        if analytics and 'monthly_summaries' in analytics:
            ...
            if monthly_summary:
                ...
                print(f"   Decay rate per day: {monthly_summary['decay_rate_per_day']:.2f}")
                print(f"   Decay percentage: {monthly_summary['decay_percentage']:.1f}%")
                print(f"   Days since calibration: {monthly_summary['days_since_calibration']}")
            else:
        ...
        if success:
            ...
            decay_summary = pipeline_service.get_pipeline_decay_summary(ae_id, month)
            if decay_summary:
                if (decay_summary['calibrated_pipeline'] == new_calibration and 
                    decay_summary['current_pipeline'] == new_calibration):
        ...
        if monthly_summaries:
            ...
            if test_month_summary:
                ...
                print(f"   Current pipeline: {test_month_summary['current_pipeline']:,}")
                print(f"   Calibrated pipeline: {test_month_summary.get('calibrated_pipeline', 'N/A')}")
                print(f"   Decay events: {test_month_summary.get('decay_events_count', 0)}")
                print(f"   Has decay activity: {test_month_summary.get('has_decay_activity', False)}")
            else:
        ...
        if os.path.exists(pipeline_file):
            try:
                ...
                print(f"   Audit log entries: {len(pipeline_data.get('audit_log', []))}")
...
def test_decay_system_integration():
    ...
    try:
        ...
        has_decay_engine = hasattr(pipeline_service, 'decay_engine') and pipeline_service.decay_engine is not None
        ...
        print(f"✅ Decay methods available: {len(available_methods)}/{len(decay_methods)}")
        ...
        print(f"✅ Data source info includes decay system: {'decay_system_enabled' in data_source_info}")
