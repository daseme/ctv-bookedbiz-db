        ...
        for test in tests:
            try:
                ...
                if count >= test['min_expected']:
                    ...
                    print(f"✗ {test['name']}: {count} records (expected >= {test['min_expected']})")
                    all_passed = False
            except Exception as e:
    ...
    def generate_summary_report(self):
        ...
        try:
            ...
            if stats:
                ...
                print(f"Average Revenue per Block: ${stats['avg_revenue_per_block']:,.2f}")
            ...
            if top_blocks:
                for i, block in enumerate(top_blocks, 1):
                    growth = f"{block['yoy_revenue_growth_pct']:.1f}%" if block['yoy_revenue_growth_pct'] else "N/A"
                    ...
                    print(f"    Growth: {growth} | Rating: {block['performance_rating']}")
                    print()
            else:
...
def main():
    ...
    if not os.path.exists(db_path):
        ...
        print("Please make sure you've downloaded the database with: python db_sync.py download")
        return False
