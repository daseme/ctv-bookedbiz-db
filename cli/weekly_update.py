...
from services.broadcast_month_import_service import (
    BroadcastMonthImportService,
    BroadcastMonthImportError,
)
from services.import_integration_utilities import (
...
class WeeklyMarketSetupManager:
    ...
    def scan_excel_for_new_markets(self, excel_file: str) -> Dict[str, Dict]:
        ...
        try:
            ...
            with tqdm(
                total=total_rows, desc="🔍 Scanning for new markets", unit=" rows"
            ) as pbar:
                for row_num, row in enumerate(
                            ...
                            if market_code not in existing_markets:
                                ...
                                if market_code not in markets_data:
                                    ...
                                    if air_date:
                                        ...
                                        if (
                                            not markets_data[market_code]["latest_date"]
                                            or air_date
                                            > markets_data[market_code]["latest_date"]
                                        ):
                                            markets_data[market_code]["latest_date"] = (
                                                air_date
                                            )
                    ...
                    if len(new_markets_found) > 0:
                        pbar.set_description(
                            f"🔍 Scanning ({len(new_markets_found)} new markets found)"
                        )
            ...
            if markets_data:
                ...
                tqdm.write(
                    f"✅ Found {len(markets_data)} new markets with {total_spots:,} spots"
                )
    ...
    def create_new_markets(self, new_markets: Dict[str, Dict]) -> Dict[str, int]:
        ...
        with self.db.transaction() as conn:
            ...
            with tqdm(
                total=len(new_markets), desc="🏗️  Creating markets", unit=" markets"
            ) as pbar:
    ...
    def setup_schedules_for_new_markets(
        self, new_markets: Dict[str, Dict], market_mapping: Dict[str, int]
    ) -> int:
    ...
    def execute_weekly_market_setup(self, excel_file: str) -> Dict:
        ...
        schedules_created = self.setup_schedules_for_new_markets(
...
class EnhancedWeeklyImporter:
    ...
    def execute_enhanced_weekly_update(
        self, excel_file: str, auto_setup_markets: bool = True, dry_run: bool = False
    ) -> Dict:
        """
        ...
        try:
            ...
            if dry_run:
                ...
                tqdm.write(
                    f"📊 Would import {summary['total_existing_spots_affected']:,} spots across {len(summary['months_in_excel'])} months"
                )
            else:
    ...
    def _execute_import_with_progress(self, excel_file: str):
        ...
        with tqdm(total=100, desc="📦 Importing data", unit="%") as pbar:
            ...
            import_result = self.import_service.execute_month_replacement(
                excel_file, "WEEKLY_UPDATE", closed_by=None, dry_run=False
            )
        ...
        if import_result.success:
            ...
            tqdm.write(
                f"📊 Import: {import_result.records_imported:,} imported, {import_result.records_deleted:,} deleted (net: {net_change:+,})"
            )
    ...
    def _process_language_assignments(self, batch_id: str) -> Dict:
        ...
        try:
            ...
            from services.language_processing_orchestrator import (
                LanguageProcessingOrchestrator,
            )
            ...
            if uncategorized_spots:
                ...
                with tqdm(
                    ...
                    for i in range(0, len(uncategorized_spots), batch_size):
                        ...
                        pbar.set_description(
                            f"🏷️  Categorized {total_categorized:,}/{len(uncategorized_spots):,}"
                        )
            ...
            language_result["default_english_assigned"] = summary[
                "default_english_assigned"
            ]
            ...
            tqdm.write(
                f"   🇺🇸 Default English: {language_result['default_english_assigned']:,}"
            )
            if language_result["flagged_for_review"] > 0:
                tqdm.write(
                    f"   📋 Review required: {language_result['flagged_for_review']:,}"
                )
...
def get_user_confirmation(
    total_spots: int, open_months: int, new_markets: int = 0, force: bool = False
) -> bool:
...
Enhanced Features:
  ...
  • Backward compatible with existing workflows
    ...
    try:
        ...
        can_proceed = display_enhanced_weekly_preview(
            args.excel_file, args.db_path, args.auto_setup
        )
        ...
        try:
            results = importer.execute_enhanced_weekly_update(
                args.excel_file, args.auto_setup, args.dry_run
            )
            ...
            if (
                results["market_setup"]
                and results["market_setup"]["markets_created"] > 0
            ):
            ...
            if results["import_result"] and not args.dry_run:
                ...
                if hasattr(import_res, "success") and import_res.success:
                    net_change = (
                        import_res.records_imported - import_res.records_deleted
                    )
            ...
            if (
                ...
                and results["language_assignment"]["success"]
            ):
