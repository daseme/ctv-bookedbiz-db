...
def build_year_conditions(year_filter: Optional[Union[str, List[str]]]) -> Tuple[str, str, str]:
    """
...
def get_database_connection(db_path: str = "data/database/production.db") -> Optional[sqlite3.Connection]:
...
def get_performance_overview_data(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> Dict[str, Any]:
...
def get_language_performance_data(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> List[Tuple]:
...
def get_top_blocks_data(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]], limit: int = 15) -> List[Tuple]:
    """Retrieve top performing blocks data - FIXED month counting"""
    cursor = conn.cursor()
    year_condition, case_condition, month_case_condition = build_year_conditions(year_filter)
...
def get_market_performance_data(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> List[Tuple]:
...
def get_time_slot_performance_data(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> List[Tuple]:
...
def get_comprehensive_block_data(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> List[Tuple]:
...
def get_bottom_daytime_blocks_data(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> List[Tuple]:
...
def get_insights_data(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> Dict[str, Any]:
...
def process_comprehensive_block_data(blocks_data: List[Tuple]) -> Dict[str, Any]:
    ...
    summary = {
        ...
        'unused_percentage': (len(unused_blocks) / total_blocks * 100) if total_blocks > 0 else 0,
        'weak_percentage': (len(weak_blocks) / total_blocks * 100) if total_blocks > 0 else 0,
        'single_customer_percentage': (len(single_customer_blocks) / total_blocks * 100) if total_blocks > 0 else 0
    }
...
def display_language_performance(languages_data: List[Tuple]) -> None:
    ...
    print("-" * 40)
    print(f"{'Language':15} {'Blocks':>7} {'Revenue':>12} {'Spots':>8} {'Revenue/Spot':>12} {'Avg Revenue':>12}")
    print("-" * 85)
    ...
    for lang in languages_data:
        ...
        print(f"{language_name:15} {blocks:>7} ${revenue:>11,.0f} {spots:>8,} ${rev_per_spot:>11.2f} ${avg_revenue:>11.2f}")
...
def display_top_blocks(blocks_data: List[Tuple], limit: int = 15) -> None:
    ...
    print("-" * 50)
    print(f"{'Block Name':25} {'Language':12} {'Market':8} {'Time Slot':15} {'Revenue':>12} {'Spots':>7} {'Months':>7}")
    print("-" * 105)
    ...
    for block in blocks_data:
        ...
        print(f"{block_name:25} {language:12} {market:8} {time_slot:15} ${revenue:>11,.0f} {spots:>7} {months:>7}")

def display_market_performance(markets_data: List[Tuple]) -> None:
    """Display market performance"""
    print(f"\n🏢 MARKET PERFORMANCE")
    print("-" * 25)
    print(f"{'Market':15} {'Code':6} {'Blocks':>7} {'Languages':>10} {'Revenue':>12} {'Spots':>8} {'Avg/Block':>10}")
    print("-" * 85)
    ...
    for market in markets_data:
        ...
        print(f"{market_name:15} {code:6} {blocks:>7} {languages:>10} ${revenue:>11,.0f} {spots:>8,} ${avg_per_block:>9,.0f}")

def display_time_slot_performance(timeslots_data: List[Tuple]) -> None:
    """Display time slot performance"""
    print(f"\n⏰ TIME SLOT PERFORMANCE")
    print("-" * 30)
    print(f"{'Time Slot':15} {'Blocks':>7} {'Revenue':>12} {'Spots':>8} {'Revenue/Block':>13} {'Avg Revenue':>12}")
    print("-" * 85)
    ...
    for slot in timeslots_data:
        ...
        print(f"{day_part:15} {blocks:>7} ${revenue:>11,.0f} {spots:>8,} ${rev_per_block:>12,.0f} ${avg_revenue:>11.2f}")
...
def display_comprehensive_blocks(processed_data: Dict[str, Any]) -> None:
    ...
    print(f"{'Block Name':25} {'Language':10} {'Mkt':4} {'Day':3} {'Time':12} {'Revenue':>10} {'Spots':>6} {'$/Spot':>7} {'Cust':>4} {'Flag':>12}")
    print("-" * 110)
    ...
    for block in processed_data['blocks']:
        ...
        print(f"{block_name:25} {language:10} {market:4} {day:3} {time_slot:12} ${revenue:>9,.0f} {spots:>6} ${rev_per_spot:>6.0f} {customers:>4} {flag_display:>12}")
    
    # Display summary
    summary = processed_data['summary']
    print(f"\n📊 PERFORMANCE SUMMARY:")
    print(f"   • Total Language Blocks: {summary['total_blocks']}")
    print(f"   • Unused Blocks: {summary['unused_count']} ({summary['unused_percentage']:.1f}%)")
    print(f"   • Weak Performing: {summary['weak_count']} ({summary['weak_percentage']:.1f}%)")
    print(f"   • Single Customer Risk: {summary['single_customer_count']} ({summary['single_customer_percentage']:.1f}%)")
...
def display_bottom_blocks(processed_data: Dict[str, Any]) -> None:
    ...
    print("-" * 55)
    print("Criteria: Day through primetime hours (6 AM - 11 PM) • Must have revenue > $0 • Must have spots assigned")
    print()
    ...
    print(f"{'Rank':>4} {'Block Name':25} {'Language':10} {'Mkt':4} {'Day':3} {'Time':12} {'Revenue':>9} {'Spots':>6} {'$/Spot':>7} {'Cust':>4} {'Rev/Mo':>7} {'Issue':15}")
    print("-" * 125)
    ...
    for processed_block in processed_data['blocks']:
        ...
        print(f"{rank:>4} {block_name:25} {language:10} {market:4} {day:3} {time_slot:12} ${revenue:>8,.0f} {spots:>6} ${rev_per_spot:>6.0f} {customers:>4} ${revenue_per_month:>6,.0f} {primary_issue:15}")
...
def display_key_insights(insights_data: Dict[str, Any], year_filter: Optional[Union[str, List[str]]]) -> None:
...
def generate_performance_overview(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> None:
...
def generate_language_performance_analysis(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> None:
...
def generate_top_performing_blocks(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]], limit: int = 15) -> None:
...
def generate_market_performance(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> None:
    """Generate Market Performance section"""
    data = get_market_performance_data(conn, year_filter)
    display_market_performance(data)

def generate_time_slot_performance(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> None:
...
def generate_comprehensive_block_performance(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> None:
...
def generate_bottom_15_daytime_blocks(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> None:
...
def generate_key_insights(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> None:
...
def run_comprehensive_analysis(conn: sqlite3.Connection, year_filter: Optional[Union[str, List[str]]]) -> None:
    ...
    print("🔧 REFACTORED: Centralized year filters, separated concerns & FIXED month counting")
    ...
    print("🔧 Code refactored: Centralized filters, separated concerns & FIXED month counting")
...
def main() -> int:
    """Main CLI interface"""
    parser = argparse.ArgumentParser(description="REFACTORED: Enhanced Language Block Analysis Tool")
    ...
    mode_group.add_argument("--compare", nargs=2, metavar=('YEAR1', 'YEAR2'), help="Compare two years")
    mode_group.add_argument("--all", action="store_true", help="Analyze all available data")
    ...
    try:
        ...
        if args.interactive:
            ...
            if args.year not in available_years:
                print(f"❌ Year {args.year} not found. Available: {', '.join(available_years)}")
                return 1
