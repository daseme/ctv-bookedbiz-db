...
class PatternMatcher(Protocol):
    ...
    def suggest_sectors(self, customer_name: str, agency_name: Optional[str] = None) -> List[SectorSuggestion]:
        """Get sector suggestions for customer"""
        ...
    
    def get_high_confidence_matches(self, customer_names: List[str], 
                                  min_confidence: float = 0.90) -> List[tuple[str, SectorSuggestion]]:
