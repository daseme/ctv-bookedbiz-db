...
class SpotValidator:
    ...
    def _validate_business_rules(self, spot: Spot, result: ValidationResult):
        ...
        if spot.end_date and spot.air_date and spot.end_date < spot.air_date:
            result.add_error(
                "end_date", "End date cannot be before air date", "INVALID_DATE_RANGE"
            )
...
class CustomerValidator:
    ...
    def validate(self, customer: Customer) -> ValidationResult:
        ...
        if not customer.normalized_name or not customer.normalized_name.strip():
            result.add_error(
                "normalized_name", "Normalized name is required", "REQUIRED_FIELD"
            )

        # Check for whitespace issues
        if (
            customer.normalized_name
            and customer.normalized_name != customer.normalized_name.strip()
        ):
            result.add_error(
                "normalized_name",
...
class BudgetValidator:
    ...
    def validate(self, budget: Budget) -> ValidationResult:
        ...
        if budget.year < 2000 or budget.year > 2100:
            result.add_error(
                "year", f"Year {budget.year} is out of valid range", "INVALID_RANGE"
            )

        if budget.month < 1 or budget.month > 12:
            result.add_error(
                "month",
                f"Month {budget.month} must be between 1 and 12",
                "INVALID_RANGE",
            )
...
class PipelineValidator:
    ...
    def validate(self, pipeline: Pipeline) -> ValidationResult:
        ...
        if pipeline.year < 2000 or pipeline.year > 2100:
            result.add_error(
                "year", f"Year {pipeline.year} is out of valid range", "INVALID_RANGE"
            )

        if pipeline.month < 1 or pipeline.month > 12:
            result.add_error(
                "month",
                f"Month {pipeline.month} must be between 1 and 12",
                "INVALID_RANGE",
            )
        ...
        if pipeline.pipeline_amount < 0:
            result.add_error(
                "pipeline_amount",
                "Pipeline amount cannot be negative",
                "INVALID_FINANCIAL_VALUE",
            )
