...
class CustomerMatchFilters:
    ...
    def from_request_args(cls, args: Dict[str, Any]) -> CustomerMatchFilters:
        ...
        return cls(
            ...
            revenue_types=args.get('revenue_types', '').split(',') if args.get('revenue_types') else [],
            status_filter=CustomerMatchStatus(args.get('status')) if args.get('status') else None,
...
class CustomerMatchingResult:
    ...
    def needs_attention_count(self) -> int:
        return len([c for c in self.candidates 
                   if c.match_status in [CustomerMatchStatus.PENDING_REVIEW, 
