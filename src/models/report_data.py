...
class CustomerMonthlyRow:
    ...
    def set_month_value(
        self, month: int, gross_value: Decimal, net_value: Decimal
    ) -> None:
    ...
    def total_gross(self) -> Decimal:
        ...
        return sum(
            getattr(self, f"month_{m}_gross", Decimal("0")) for m in range(1, 13)
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for month in range(1, 13):
            ...
            result[f"month_{month}"] = float(
                self.get_month_gross(month)
            )  # Default to gross
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_customers_by_sector": [
                c.to_dict() for c in self.top_customers_by_sector
            ],
            "total_revenue": float(self.total_revenue),
...
def create_month_status_from_closure_data(
    closures: List[Dict[str, Any]], year: int
) -> List[MonthStatus]:
        ...
        if broadcast_month in closure_map:
            ...
            status = MonthStatus(
                ...
                closed_date=datetime.strptime(
                    closure["closed_date"], "%Y-%m-%d"
                ).date(),
                closed_by=closure["closed_by"],
            )
        else:
            status = MonthStatus(
                month_number=month, month_name=month_name, status="OPEN"
            )
