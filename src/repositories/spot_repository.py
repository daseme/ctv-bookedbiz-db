...
class SpotRepository:
    ...
    def get_month_summaries(
        ...
        for row in cursor.fetchall():
            results[row[0]] = MonthSummary(
                month=row[0], count=row[1], gross_revenue=row[2]
            )
    ...
    def get_record_count_by_month(self, month: str, conn: sqlite3.Connection) -> int:
    ...
    def delete_by_months(self, months: List[str], conn: sqlite3.Connection) -> int:
        """
    ...
    def insert_spot(
        self, spot_data: Dict[str, Any], conn: sqlite3.Connection
    ) -> Optional[int]:
        """
        ...
        cursor = conn.execute(
            f"INSERT INTO spots ({field_names}) VALUES ({placeholders})", values
        )
