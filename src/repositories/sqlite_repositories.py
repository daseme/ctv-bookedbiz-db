...
class SQLiteSpotRepository(SpotRepository, BaseService):
    ...
    def save(self, spot: Spot) -> Spot:
        ...
        with self.safe_transaction() as conn:
            if spot.spot_id is None:
                ...
                spot_dict = self._spot_to_dict(spot)
                set_clause = ", ".join(
                    [f"{k} = ?" for k in spot_dict.keys() if k != "spot_id"]
                )
    ...
    def get_revenue_by_ae_and_month(
    ...
    def _row_to_spot(self, row: sqlite3.Row) -> Spot:
        ...
        return Spot(
            ...
            broker_fees=Decimal(str(row["broker_fees"]))
            if row["broker_fees"]
            else None,
            priority=row["priority"],
            station_net=Decimal(str(row["station_net"]))
            if row["station_net"]
            else None,
            ...
            load_date=datetime.fromisoformat(row["load_date"])
            if row["load_date"]
            else None,
...
class SQLiteCustomerRepository(CustomerRepository, BaseService):
    ...
    def save(self, customer: Customer) -> Customer:
        ...
        with self.safe_transaction() as conn:
            if customer.customer_id is None:
                ...
                logger.info(
                    f"Created new customer: {customer.normalized_name} (ID: {customer.customer_id})"
                )
            else:
                ...
                logger.info(
                    f"Updated customer: {customer.normalized_name} (ID: {customer.customer_id})"
                )
    ...
    def find_similar_customers(
        ...
        with self.safe_connection() as conn:
            query = (
                "SELECT * FROM customers WHERE normalized_name LIKE ? AND is_active = 1"
            )
    ...
    def get_all_customers(self) -> List[Customer]:
        ...
        with self.safe_connection() as conn:
            query = (
                "SELECT * FROM customers WHERE is_active = 1 ORDER BY normalized_name"
            )
        ...
        with self.safe_connection() as conn:
            ...
            for row in cursor.fetchall():
                ...
                if not agency_name and row["sample_bill_codes"]:
                    agency_name = self._extract_agency_from_bill_codes(
                        row["sample_bill_codes"]
                    )
    ...
    def assign_sector_to_customer(
        ...
        try:
            ...
            with self.safe_connection() as conn:
                cursor = conn.execute(
                    "SELECT sector_id FROM customers WHERE customer_id = ?",
                    (customer_id,),
                )
                row = cursor.fetchone()
                if row:
            ...
            with self.safe_transaction() as conn:
                ...
                self._create_audit_entry(
                    conn, customer_id, old_sector_id, sector_id, method, assigned_by
                )

                logger.info(
                    f"Assigned sector {sector_id} to customer {customer_id} via {method.value}"
                )
...
class ReferenceDataRepository(BaseService):
    ...
    def get_or_create_agency(self, agency_name: str) -> Agency:
        ...
        with self.safe_transaction() as conn:
            ...
            cursor = conn.execute(
                "SELECT * FROM agencies WHERE agency_name = ?", (agency_name,)
            )
            row = cursor.fetchone()
    ...
    def get_all_agencies(self) -> List[Agency]:
        ...
        with self.safe_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM agencies WHERE is_active = 1 ORDER BY agency_name"
            )
    ...
    def get_market_by_code(self, market_code: str) -> Optional[Market]:
        ...
        with self.safe_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM markets WHERE market_code = ?", (market_code,)
            )
            row = cursor.fetchone()
    ...
    def get_all_markets(self) -> List[Market]:
        ...
        with self.safe_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM markets WHERE is_active = 1 ORDER BY market_name"
            )
    ...
    def get_all_sectors(self) -> List[Sector]:
        ...
        with self.safe_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM sectors WHERE is_active = 1 ORDER BY sector_name"
            )
    ...
    def get_language_by_code(self, language_code: str) -> Optional[Language]:
        ...
        with self.safe_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM languages WHERE language_code = ?", (language_code,)
            )
            row = cursor.fetchone()
