...
class SectorPlanningRepository(BaseService):
    ...
    def get_booked_by_sector(self, ae_name: str, year: int) -> EntitySectorBooked:
        ...
        for row in rows:
            ...
            records.append(SectorBookedData(
                sector_id=row[0],
                sector_code=row[1],
                sector_name=row[2],
                month=month_num,
        ...
        return EntitySectorBooked(
            ae_name=ae_name,
            year=year,
            records=records
        )
    
    def get_booked_by_sector_all_entities(self, year: int) -> Dict[str, EntitySectorBooked]:
        """
        ...
        for row in rows:
            ...
            month_num = month_map.get(month_abbr[:3], 0) if month_abbr else 0
            result[ae].append(SectorBookedData(
                sector_id=row[1],
                sector_code=row[2],
                sector_name=row[3],
                month=month_num,
