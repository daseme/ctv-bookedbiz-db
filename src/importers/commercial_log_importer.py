...
class ExcelSheetReader:
    ...
    def read_multiple_sheets(
        self, source_path: str, sheet_names: List[str]
    ) -> Dict[str, SheetProcessingResult]:
        ...
        try:
            ...
            with pd.ExcelFile(source_path) as xl_file:
                ...
                for sheet_name in sheet_names:
                    results[sheet_name] = self._read_single_sheet(
                        xl_file, sheet_name, available_sheets
                    )
    ...
    def _read_single_sheet(
        self, xl_file, sheet_name: str, available_sheets: List[str]
        ...
        try:
            ...
            if len(df) > 0:
                self.logger.info(
                    f"Successfully read '{sheet_name}' sheet: {len(df)} rows, {len(df.columns)} columns"
                )
...
class ExcelDataCombiner:
    ...
    def combine_sheets(self, sheet_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        self.logger.info(
            f"Combined dataset: {len(combined_df):,} total rows from {len(combined_dataframes)} sheets"
        )
        return combined_df
...
class MultiSheetCommercialLogService:
    ...
    def process_commercial_log_file(
        ...
        sheet_results = self.sheet_reader.read_multiple_sheets(
            source_path, CommercialLogConfig.REQUIRED_SHEETS
        )

        # Check if any sheets failed
        failed_sheets = [
            result for result in sheet_results.values() if not result.success
        ]
        if failed_sheets:
        ...
        write_success = self.file_writer.write_combined_data(
            combined_df, destination_path
        )
...
class CommercialLogImportFacade:
    ...
    def __init__(self):
        ...
        self.commercial_log_service = MultiSheetCommercialLogService(
            self.sheet_reader, self.data_combiner, self.file_writer, self.logger
        )
    ...
    def copy_commercial_log(self) -> bool:
        ...
        """
        self.logger.info(
            "Starting Multi-Sheet Commercial Log copy process - PRODUCTION MODE"
        )
        ...
        # Validate inputs
        if not FileSystemService.validate_source_file(
            CommercialLogConfig.SOURCE_FILE, self.logger
        ):
            return False

        if not FileSystemService.ensure_destination_directory(
            CommercialLogConfig.DESTINATION_DIR, self.logger
        ):
            return False

        # Process multi-sheet file
        result = self.commercial_log_service.process_commercial_log_file(
            CommercialLogConfig.SOURCE_FILE, destination_path
        )
...
def read_commercials_sheet(
    source_path: str, logger: logging.Logger
    ...
    # Read both sheets
    sheet_results = facade.sheet_reader.read_multiple_sheets(
        source_path, CommercialLogConfig.REQUIRED_SHEETS
    )
    ...
    if failed_sheets:
        logger.error(
            f"Failed to read required sheets: {[s.sheet_name for s in failed_sheets]}"
        )
        return None
...
def write_excel_file(
    df: pd.DataFrame, destination_path: str, logger: logging.Logger
) -> bool:
    """Legacy function - maintained for compatibility"""
