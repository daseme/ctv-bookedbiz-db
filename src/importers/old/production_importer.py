...
class ProductionExcelImporter:
    ...
    def import_excel_file(
        self, excel_file_path: str, limit: Optional[int] = None
    ) -> ImportResults:
    ...
    def _process_row(
        self, row: tuple, row_num: int, db_conn: sqlite3.Connection
    ) -> bool:
        ...
        try:
            ...
            # Handle agency
            if agency_name:
                spot_data["agency_id"] = self._get_or_create_agency(
                    db_conn, agency_name
                )

            # Handle customer
            spot_data["customer_id"] = self._get_or_create_customer(
                db_conn, customer_name
            )

            # Handle market mapping
            if spot_data.get("market_name"):
                spot_data["market_id"] = self._get_market_id(
                    db_conn, spot_data["market_name"]
                )
    ...
    def _get_or_create_customer(
        self, conn: sqlite3.Connection, customer_name: str
    ) -> int:
    ...
    def _get_market_id(
        self, conn: sqlite3.Connection, market_name: str
    ) -> Optional[int]:
        """Get market ID."""
        cursor = conn.execute(
            "SELECT market_id FROM markets WHERE LOWER(market_name) = LOWER(?)",
            (market_name,),
        )
        row = cursor.fetchone()
    ...
    def _insert_spot(
        self, conn: sqlite3.Connection, spot_data: Dict[str, Any], source_file: str
    ):
    ...
    def _record_error(self, context: str, error: str):
        ...
        error_type = (
            type(error).__name__ if hasattr(error, "__name__") else "ProcessingError"
        )
