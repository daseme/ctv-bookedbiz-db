...
class ProgressTracker:
    ...
    def update(self, imported: bool = True):
        ...
        if self.memory_monitoring:
            try:
                ...
                if (
                    current_memory - self.initial_memory > 500
                    and self.processed % 10000 == 0
                ):
                    gc.collect()
            except:
                pass
        ...
        if (
            self.processed - self.last_update >= self.update_interval
    ...
    def _display_progress(self):
        ...
        percent = (
            (self.processed / self.total_records) * 100 if self.total_records > 0 else 0
        )

        progress_text = (
            f"\rProgress: {self.processed:,}/{self.total_records:,} ({percent:.1f}%) "
        ...
        if self.memory_monitoring:
            try:
                ...
                progress_text += (
                    f" | Memory: {current_memory:.0f}MB (+{memory_used:.0f}MB)"
                )
            except:
                pass
...
class EnhancedProductionExcelImporter:
    ...
    def validate_and_confirm_column_mapping(self, worksheet, interactive=True):
        ...
        actual_headers = [
            str(header).strip() if header else "" for header in header_row
        ]
    ...
    def import_with_batch_id(
        ...
        try:
            ...
            records_imported = self._process_sequential_fixed(
                worksheet, total_rows, batch_id, progress, limit
            )
            ...
            results = ImportResults(
                ...
                batch_id=batch_id,
                broadcast_months_processed=sorted(
                    list(self.stats["broadcast_months_found"])
                ),
            )
    ...
    def _preload_database_caches(self):
        ...
        try:
            ...
            print(
                f"✓ Pre-loaded caches: {len(self._agency_cache)} agencies, {len(self._customer_cache)} customers, {len(self._market_cache)} markets"
            )
    ...
    def _parse_headers(self, worksheet, interactive=False):
        ...
        for field in required_fields:
            if field not in self.column_indexes:
                original_name = next(
                    (k for k, v in self.COLUMN_MAPPING.items() if v == field), field
                )
    ...
    def _process_row_fixed(
        ...
        try:
            ...
            for field_name, col_idx in self.column_indexes.items():
                if col_idx < len(row):
                    spot_data[field_name] = self._convert_value(
            ...
            if spot_data.get("broadcast_month"):
                ...
                if isinstance(bm, (datetime, date)):
                    spot_data["broadcast_month"] = normalize_broadcast_day(
                        bm
                        if isinstance(bm, datetime)
                        else datetime.combine(bm, datetime.min.time())
                    )
                ...
                try:
                    ...
                    display = (
                        BroadcastMonthParser().parse_excel_date_to_broadcast_month(
            ...
            spot_data["customer_id"] = self._get_or_create_customer_cached(
                customer_name
            )

            # Market cache
            if spot_data.get("market_name"):
                spot_data["market_id"] = self._get_market_id_cached(
                    spot_data["market_name"]
                )
    ...
    def _get_or_create_agency_cached(self, agency_name: str) -> int:
        ...
        try:
            conn = sqlite3.connect(self.database_path)
            cursor = conn.execute(
                "INSERT INTO agencies (agency_name) VALUES (?)", (agency_name,)
            )
            ...
            conn = sqlite3.connect(self.database_path)
            cursor = conn.execute(
                "SELECT agency_id FROM agencies WHERE agency_name = ?", (agency_name,)
            )
            row = cursor.fetchone()
            conn.close()
            if row:
    ...
    def _get_or_create_customer_cached(self, customer_name: str) -> int:
        ...
        try:
            conn = sqlite3.connect(self.database_path)
            cursor = conn.execute(
                "INSERT INTO customers (normalized_name) VALUES (?)", (customer_name,)
            )
...
def import_excel_with_batch(
    excel_file_path: str, database_path: str, batch_id: str
) -> int:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="FIXED Enhanced Production Excel Importer"
    )
    ...
    parser.add_argument(
        "--limit", type=int, help="Limit number of records (for testing)"
    )
