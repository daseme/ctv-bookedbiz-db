...
class SmartFilteredImporter(BaseService):
    ...
    def analyze_workbook(
        self, excel_file: str, target_year: int = None
    ) -> Dict[str, Any]:
        """
    ...
    def _analyze_excel_structure(
        self, source_excel: str, target_months: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        # Find Month column
        header_row = list(source_ws.iter_rows(min_row=1, max_row=1, values_only=True))[
            0
        ]
        ...
        with tqdm(total=total_rows, desc="Analyzing structure", unit=" rows") as pbar:
            for row_num, row in enumerate(
                source_ws.iter_rows(min_row=2, values_only=True), start=2
            ):
                pbar.update(1)
                ...
                if month_col_index < len(row):
                    ...
                    if month_value is not None:
                        try:
                            display_month = parser.parse_excel_date_to_broadcast_month(
                                month_value
                            )
    ...
    def _create_filtered_excel(
        self, source_excel: str, analysis: Dict[str, Any], temp_file: str
    ) -> str:
        """
            ...
            for row_num, row in enumerate(
                ...
                if row_num % 5000 == 0:
                    pbar.set_description(
                        f"Copied: {rows_copied:,} | Skipped: {rows_skipped:,}"
                    )
        ...
        print(
            f"Filtered Excel created: {rows_copied:,} rows copied, {rows_skipped:,} rows skipped"
        )
        return temp_file

    def create_filtered_excel(
        self, source_excel: str, target_months: List[str], temp_file: str
    ) -> str:
        """
    ...
    def execute_filtered_import(
        self,
        excel_file: str,
        target_year: int,
        closed_by: str,
        dry_run: bool = False,
        close_imported_months: bool = True,
        ...
        try:
            ...
            temp_excel = (
                f"temp_filtered_{target_year}_{int(start_time.timestamp())}.xlsx"
            )
            ...
            try:
                ...
                if import_result.success:
                    ...
                    if result.months_closed:
                        print(
                            f"   Months closed: {len(result.months_closed)} {result.months_closed}"
                        )
...
Examples:
  ...
  python smart_import.py data/raw/2025_complete.xlsx --year 2025 --closed-by "Kurt" --no-close
    ...
    parser.add_argument(
        "--year", type=int, required=True, help="Target year (e.g., 2025)"
    )
    parser.add_argument(
        "--closed-by", required=True, help="Name of person performing import"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-close", action="store_true", help="Don't close months after import"
    )
