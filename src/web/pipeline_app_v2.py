...
from src.services.db_sync_decay_service import DatabaseSyncDecayService, enhance_ae_service_with_auto_sync
...
ae_service = enhance_ae_service_with_auto_sync(original_ae_service, pipeline_service)  # Enhanced
...
def get_ae_summary(ae_id: str):
    ...
    try:
        ...
        for month_num in range(1, 13):  # All 12 months
            ...
            pipeline_values = PipelineCalculator.calculate_pipeline_values(budget, booked_revenue, month_str, assigned_pipeline)
        ...
        for quarter in quarters:
            quarter_months = [m for m in monthly_summary if int(m['month'].split('-')[1]) in quarter['months']]
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f"Initial sync: {results['decay_adjustments_applied']} decay adjustments applied")
    except Exception as e:
