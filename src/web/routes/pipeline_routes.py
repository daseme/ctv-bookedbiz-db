...
class PipelineCalculator:
    ...
    @staticmethod
    def calculate_pipeline_values(budget: float, booked_revenue: float = 0, month_str: str = None, assigned_pipeline: float = None) -> Dict[str, float]:
    ...
    @staticmethod
    def calculate_gaps(booked_revenue: float, current_pipeline: float, 
                      expected_pipeline: float, budget: float) -> Dict[str, float]:
...
def pipeline_revenue_management():
    ...
    try:
        ...
        monthly_data = report_service.get_monthly_revenue_report_data(current_year, filters)
        ...
        if monthly_data and hasattr(monthly_data, 'to_dict'):
            ...
            if 'ae_list' in monthly_dict and monthly_dict['ae_list']:
                ...
                print(f"✅ Converted {len(ae_list)} AEs: {[ae['name'] for ae in ae_list]}")
            ...
            # If ae_list is empty, try revenue_data structure
                ...
                if isinstance(revenue_data, list) and len(revenue_data) > 0:
                    ...
                    for i, item in enumerate(revenue_data[:3]):  # Check first 3
                        print(f"🔍 Revenue item {i}: {list(item.keys()) if isinstance(item, dict) else type(item)}")
                        if isinstance(item, dict):
                            ...
                            if ae_keys:
                                ...
                                if ae_value and ae_value not in [ae['name'] for ae in ae_list]:
                                    ae_list.append({
        ...
        if not ae_list:
            ...
            ae_list = [
                {'ae_id': 'sarah_johnson', 'name': 'Sarah Johnson ⚡', 'decay_enabled': True, 'has_decay_activity': True},
                {'ae_id': 'mike_chen', 'name': 'Mike Chen', 'decay_enabled': True, 'has_decay_activity': False},
                {'ae_id': 'lisa_rodriguez', 'name': 'Lisa Rodriguez', 'decay_enabled': False, 'has_decay_activity': False},
                {'ae_id': 'david_kim', 'name': 'David Kim ⚡', 'decay_enabled': True, 'has_decay_activity': True}
            ]
        ...
        return render_template('./pipeline/pipeline_revenue_management.html',
        ...
        # Final fallback
        ae_list = [
            {'ae_id': 'demo_ae_1', 'name': 'Demo AE #1 ⚡', 'decay_enabled': True, 'has_decay_activity': True},
            {'ae_id': 'demo_ae_2', 'name': 'Demo AE #2', 'decay_enabled': True, 'has_decay_activity': False}
        ]
        ...
        return render_template('./pipeline/pipeline_revenue_management.html',
                             title="Pipeline Revenue Management", data=data)
...
def get_customer_data(ae_id: str, month: str):
    ...
    try:
        ...
        logger.info(f"Getting customer data for AE: {ae_name} (ID: {ae_id}), Month: {month}")
                ...
                if item_ae == ae_name:  # Exact match
                    ...
                    if month_revenue is not None:
                        ...
                        if month_revenue > 0:
                            ...
                            customer_info = {
                                'customer_name': item.get('customer', 'Unknown Customer'),
        ...
        try:
            ...
            if current_pipeline > 5000:  # Only show pipeline if substantial
                ...
                pipeline_deals = [deal for deal in pipeline_deals if deal['amount'] >= 1000]
                
        except Exception as e:
            logger.warning(f"Could not get pipeline deals for {ae_id} {month}: {e}")
            pipeline_deals = []
        
        logger.info(f"Generated {len(pipeline_deals)} pipeline deals for {ae_name} in {month}")
        ...
        return create_success_response({
            ...
            'summary': {
                ...
                'total_booked_revenue': sum(deal['total_revenue'] for deal in booked_deals),
                'total_pipeline_value': sum(deal['amount'] for deal in pipeline_deals)
            }
        })
...
def get_decay_timeline(ae_id: str, month: str):
    ...
    try:
        ...
        try:
            ...
            return create_success_response({
                'timeline': [],
                'message': 'Decay system not available'
            })
