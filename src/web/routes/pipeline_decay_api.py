...
decay_api_bp = Blueprint('pipeline_decay_api', __name__, url_prefix='/api/pipeline/decay')
...
def get_decay_summary(ae_id: str, month: str):
    ...
    try:
        ...
        if decay_summary:
            ...
            return create_error_response(f"No decay data found for {ae_id} {month}", 404)
...
def get_ae_summary_with_decay(ae_id: str):
    ...
    try:
        ...
        active_months = [s for s in monthly_summaries if s.get('has_decay_activity', False)]
        
        aggregate_metrics = {
            'total_decay': total_decay,
            'active_months_count': len(active_months),
            'total_months': len(monthly_summaries),
            'avg_decay_per_month': total_decay / len(monthly_summaries) if monthly_summaries else 0,
            'most_active_month': max(active_months, key=lambda x: len(x.get('decay_events', [])))['month'] if active_months else None
        }
...
def webhook_revenue_change():
    ...
    try:
        ...
        if data['change_type'] not in valid_change_types:
            return create_error_response(f"Invalid change_type. Must be one of: {valid_change_types}", 400)
        ...
        if success:
            # Get updated decay summary
            decay_summary = pipeline_service.get_pipeline_decay_summary(ae_id, data['month'])
...
def cleanup_old_events():
    ...
    try:
        ...
        if hasattr(pipeline_service, 'decay_engine') and pipeline_service.decay_engine:
            events_removed = pipeline_service.decay_engine.cleanup_old_decay_events(days_to_keep)
...
def get_system_status():
    ...
    try:
        ...
        status = {
            ...
            'decay_system_enabled': hasattr(pipeline_service, 'decay_engine') and pipeline_service.decay_engine is not None,
        ...
        if pipeline_service.decay_engine:
            try:
                ...
                status.update({
                    ...
                    'total_months_tracked': sum(
                        len(ae_data) for ae_data in decay_data.get('decay_tracking', {}).values()
                    ),
                    ...
                    'data_source': pipeline_service.data_source.value if hasattr(pipeline_service, 'data_source') else 'unknown'
                })
                
                # Calculate recent activity
                recent_events = 0
                cutoff_date = (datetime.utcnow().replace(day=1) - 
                             datetime.utcnow().replace(day=1, month=datetime.utcnow().month-1 if datetime.utcnow().month > 1 else 12)).isoformat() + "Z"
                ...
                status['recent_activity'] = {
                    ...
                    'activity_level': 'high' if recent_events > 50 else 'medium' if recent_events > 10 else 'low'
                }
            ...
            status.update({
                ...
                'message': 'Decay system not initialized'
            })
...
def export_decay_data(ae_id: str):
    ...
    try:
        ...
        if export_format == 'csv':
            ...
            return create_error_response(f"Unsupported export format: {export_format}", 400)
...
def _export_decay_as_csv(analytics: dict, ae_id: str):
    ...
    # Write headers
    writer.writerow(['AE_ID', 'Month', 'Calibrated_Pipeline', 'Current_Pipeline', 
                    'Total_Decay', 'Decay_Rate_Per_Day', 'Decay_Percentage', 
                    'Days_Since_Calibration', 'Event_Count'])
...
def decay_api_internal_error(error):
    ...
    return create_error_response(
        "Decay API internal error",
        status_code=500,
        error_code="DECAY_INTERNAL_ERROR"
    )