...
class YearRange:
    ...
    def from_input(cls, year_input: str) -> 'YearRange':
        ...
        display = f"{full_years[0]}-{full_years[-1]}" if len(full_years) > 1 else full_years[0]
...
class BusinessCategory(Enum):
    ...
    DIRECT_RESPONSE = ("Direct Response Sales", "revenue_type = 'Direct Response Sales'")
...
class SpotRepository:
    ...
    def get_category_data(self, category: BusinessCategory, year_range: YearRange) -> AnalysisResult:
    ...
    def get_language_data(self, year_range: YearRange) -> List[AnalysisResult]:
        ...
        for lang_group in LanguageGroup:
            names_condition = " OR ".join([f"l.language_name = '{name}'" for name in lang_group.language_names])
            language_cases.append(f"WHEN ({names_condition}) THEN '{lang_group.display_name}'")
        
        language_case_sql = "CASE " + " ".join(language_cases) + " ELSE 'Other: Undetermined' END"
...
class AnalysisService:
    ...
    def get_category_analysis(self, year_input: str) -> List[AnalysisResult]:
        ...
        for result in results:
            result.percentage = (result.revenue / total_revenue * 100) if total_revenue else 0
...
class ReportFormatter:
    ...
    def format_analysis_table(self, results: List[AnalysisResult], title: str, 
                             subtitle: str, year_display: str) -> str:
...
"""
    ...
    def generate_full_report(self, year_input: str, analysis_service: AnalysisService) -> str:
        ...
        category_table = self.format_analysis_table(
            categories, "📊 Business Rule Category Breakdown", 
            "Revenue Categories", year_range.display
        )
        language_table = self.format_analysis_table(
            languages, "🌐 Language-Targeted Advertising", 
            "Internal Ad Sales by Language", year_range.display
        )
...
class AnalysisEngine:
    ...
    def generate_report(self, year_input: str = "2024") -> str:
        ...
        return self._report_formatter.generate_full_report(year_input, self._analysis_service)
...
def main():
    ...
    parser.add_argument("--year", default="2024", help="Year or range, e.g. 2024 or 2023-2024")
    parser.add_argument("--db-path", default="data/database/production.db", help="SQLite DB path")
    parser.add_argument("--output", help="Write report to file")
    parser.add_argument("--validate-only", action="store_true", help="Only run reconciliation")
    ...
    try:
        with AnalysisEngine(args.db_path) as engine:
            if args.validate_only:
                ...
                print(f"Perfect: {'YES' if validation['perfect_reconciliation'] else 'NO'}")
                return
