...
ADMIN_REGEX = re.compile(r"\b(?:" + "|".join(ADMIN_PATTERNS) + r")\b", re.IGNORECASE)
...
def identify_admin_entries(
    bill_codes: List[Tuple[str, float, int]],
...
def is_suspicious_pattern(bill_code: str) -> bool:
    ...
    suspicious_indicators = [
        ...
        len(bill_code) < 10
        and bill_code.isupper()
        and not any(
            word.lower() in ["inc", "llc", "corp", "ltd"] for word in bill_code.split()
        ),
...
def print_analysis(categorized: Dict[str, List[Tuple[str, float, int]]]):
    ...
    print(
        f"Admin entries:        {len(admin_entries):,} ({len(admin_entries) / total_entries * 100:.1f}%)"
    )
    print(
        f"Suspicious entries:   {len(suspicious_entries):,} ({len(suspicious_entries) / total_entries * 100:.1f}%)"
    )
    print(
        f"Clean entries:        {len(clean_entries):,} ({len(clean_entries) / total_entries * 100:.1f}%)"
    )
    print()
    print(f"Total revenue:        ${total_revenue:,.2f}")
    print(
        f"Admin revenue:        ${admin_revenue:,.2f} ({admin_revenue / total_revenue * 100:.1f}%)"
    )
    print(
        f"Suspicious revenue:   ${suspicious_revenue:,.2f} ({suspicious_revenue / total_revenue * 100:.1f}%)"
    )
    print(
        f"Clean revenue:        ${clean_revenue:,.2f} ({clean_revenue / total_revenue * 100:.1f}%)"
    )
    ...
    print("-" * 80)
    for bill_code, revenue, spots in sorted(
        admin_entries, key=lambda x: x[1], reverse=True
    )[:20]:
    ...
    if suspicious_entries:
        ...
        print("-" * 80)
        for bill_code, revenue, spots in sorted(
            suspicious_entries, key=lambda x: x[1], reverse=True
        )[:20]:
            print(f"{bill_code[:49]:<50} | ${revenue:>10,.0f} | {spots:>8,}")
...
def main():
    ...
    if args.min_revenue > 0:
        bill_codes = [
            (bc, rev, spots) for bc, rev, spots in bill_codes if rev >= args.min_revenue
        ]
    ...
    print("\nRECOMMENDATIONS:")
    print(
        "1. Review the 'admin' entries - these likely should be excluded from customer matching"
    )
    print(
        "2. Check 'suspicious' entries manually - some may be legitimate, others may be admin"
    )
    print(
        "3. Consider adding a filter to exclude admin entries from your matching algorithm"
    )
