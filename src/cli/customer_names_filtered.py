...
def print_filter_summary(filter_stats: dict) -> None:
    ...
    print(f"Admin entries (excluded):  {admin_count:,} ({admin_count/total*100:.1f}%)")
    print(f"Suspicious entries (kept): {suspicious_count:,} ({suspicious_count/total*100:.1f}%)")
    print(f"Clean entries (kept):      {clean_count:,} ({clean_count/total*100:.1f}%)")
    print()
    ...
    print(f"Admin revenue (excluded):  ${admin_revenue:,.2f} ({admin_revenue/total_revenue*100:.1f}%)")
    print(f"Clean + Suspicious kept:   ${clean_revenue + suspicious_revenue:,.2f} ({(clean_revenue + suspicious_revenue)/total_revenue*100:.1f}%)")
    print()
    
    # Show top excluded admin entries
    admin_entries = sorted(filter_stats['admin_entries'], key=lambda x: abs(x[1]), reverse=True)
...
def build_parser():
    ...
    p.add_argument("--suggest-aliases", action="store_true", help="Print suggested aliases")
    p.add_argument("--alias-min-revenue", type=float, default=1000.0, help="Min revenue for alias suggestions")
    p.add_argument("--alias-min-score", type=float, default=0.85, help="Min score for alias suggestions")
    p.add_argument("--show-filtered", action="store_true", help="Show what was filtered out")
    return p
...
def _run_filtered_analysis(db_path: str,
                          limit: int,
                          do_export: bool,
    ...
    if show_filtered:
        ...
        print(f"Excluded {admin_count} admin entries ({admin_count/total_count*100:.1f}% of total)")
        print()
    ...
    for match in matches:
        ...
        bill_code_name = getattr(match, 'bill_code_name_raw', '') or getattr(match, 'original_name', '')
    ...
    if do_suggest:
        suggest_alias_sql(filtered_matches, min_revenue=alias_min_revenue, min_score=alias_min_score)
