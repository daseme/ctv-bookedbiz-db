...
class BusinessRulesCLI:
    ...
    def estimate_impact(self) -> None:
        ...
        overall_percentage = (total_affected / total_spots * 100) if total_spots > 0 else 0
        print("-" * 55)
        print(f"{'TOTAL AFFECTED':<30} {total_affected:<12,} {overall_percentage:<12.1f}%")
    ...
    def show_rules(self) -> None:
        ...
        for i, rule in enumerate(rules, 1):
            ...
            print(f"   Sectors: {rule['sector_codes'] if rule['sector_codes'] else 'Any'}")
    ...
    def _print_test_results(self, results: List[tuple]) -> None:
        ...
        for spot_data, result in results:
            if result.rule_applied and shown < 5:
                print(f"   • Spot {spot_data.spot_id}: {spot_data.customer_name[:40]}...")
...
Examples:
  ...
  python -m src.cli.business_rules_cli --export rules.json       # Export rules
    ...
    action_group.add_argument("--rules", action="store_true", help="Show all business rules")
    action_group.add_argument("--test", type=int, metavar="N", help="Test rules on N spots")
    action_group.add_argument("--estimate", action="store_true", help="Estimate impact of rules")
    action_group.add_argument("--validate", action="store_true", help="Validate rules configuration")
    action_group.add_argument("--stats", action="store_true", help="Show statistics")
    action_group.add_argument("--export", metavar="FILE", help="Export rules to JSON file")
