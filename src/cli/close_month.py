...
def display_all_closed_months(service: MonthClosureService):
    ...
    try:
        ...
        for info in closed_months:
            ...
            display_name = parser.format_broadcast_month_for_display(
                info["broadcast_month"]
            )
...
def get_closure_confirmation(month: str, stats: dict) -> bool:
    ...
    print("  • Prevent future modifications to this month")
    print("  • Make closure PERMANENT (cannot be undone)")
    ...
    while True:
        response = input(
            f"\nPermanently close {month}? (type 'CLOSE' to confirm): "
        ).strip()
        if response == "CLOSE":
            return True
...
Examples:
  ...
  python src/cli/close_month.py --status "May-25"
    ...
    parser.add_argument(
        "--closed-by", help="Name/ID of person closing the month (required for closure)"
    )
    parser.add_argument("--notes", help="Optional notes about the closure")
    parser.add_argument(
        "--db-path",
    ...
    parser.add_argument(
        "--list", action="store_true", help="List all currently closed months"
    )
    parser.add_argument("--status", help="Show detailed status for a specific month")
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    try:
        if args.list:
            ...
            if service.is_month_closed(month):
                ...
                if closure_info:
                    print(
                        f"Closed on: {closure_info['closed_date']} by {closure_info['closed_by']}"
                    )
            ...
            if stats["total_spots"] == 0:
                ...
                print("Import data first, then close the month")
                sys.exit(1)
            ...
            if not args.force:
                ...
                if not confirmed:
                    print("❌ Month closure cancelled by user")
                    sys.exit(0)

            # Execute the closure
            try:
                success = service.close_broadcast_month(
                    month, args.closed_by, args.notes
                )
                ...
                if success:
                    ...
                    print(
                        f"  Revenue protected: ${updated_stats['total_revenue']:,.2f}"
                    )

                    print("\n💡 Next steps:")
                    print("  • Month is now permanently protected from changes")
                    print("  • Continue with weekly updates as normal")
                    print(
                        "  • View all closed months: uv run python src/cli/close_month.py --list"
                    )
                else:
            ...
            print("\nQuick commands:")
            print(
                "  List closed months:    uv run python src/cli/close_month.py --list"
            )
            print(
                "  Check month status:    uv run python src/cli/close_month.py --status 'May-25'"
            )
            print(
                "  Close a month:         uv run python src/cli/close_month.py 'May-25' --closed-by 'Kurt'"
            )
