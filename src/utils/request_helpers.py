...
def create_json_response(
    ...
    try:
        ...
        response = Response(
            json_data, status=status_code, mimetype="application/json", headers=headers
        )
...
def handle_service_error(error: Exception, operation: str) -> Response:
    ...
    if "not found" in str(error).lower():
        return create_error_response(
            f"Resource not found: {operation}", status_code=404, error_code="NOT_FOUND"
        )
    elif "validation" in str(error).lower():
        return create_error_response(
            f"Validation error: {error}", status_code=400, error_code="VALIDATION_ERROR"
        )
    elif "permission" in str(error).lower() or "unauthorized" in str(error).lower():
        return create_error_response(
            f"Access denied: {operation}", status_code=403, error_code="ACCESS_DENIED"
        )
    else:
        # Generic server error
        return create_error_response(
            f"Service error: {operation}", status_code=500, error_code="SERVICE_ERROR"
        )
