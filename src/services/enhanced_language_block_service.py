...
from ..models.business_rules_models import (
    BusinessRuleResult, SpotData, AssignmentResult, 
    AssignmentMethod, CustomerIntent
)
...
class EnhancedLanguageBlockService:
    ...
    def assign_single_spot(self, spot_id: int) -> AssignmentResult:
        ...
        try:
            ...
            if not spot_data:
                return self._create_error_result(spot_id, "Spot not found or invalid data")
            ...
            if rule_result.auto_resolved:
                ...
                assignment_result = self._create_business_rule_assignment(spot_data, rule_result)
                ...
                assignment_result = self._create_flagged_assignment(spot_data, rule_result)
                self._save_assignment(assignment_result)
                return assignment_result
    ...
    def assign_spots_batch(self, spot_ids: List[int] = None, limit: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Processing {len(spots_to_process)} spots with business rules")
    ...
    def _create_business_rule_assignment(self, spot_data: SpotData, rule_result: BusinessRuleResult) -> AssignmentResult:
    ...
    def _create_flagged_assignment(self, spot_data: SpotData, rule_result: BusinessRuleResult) -> AssignmentResult:
        ...
        return AssignmentResult(
            ...
            alert_reason=rule_result.alert_reason,
            assignment_method=AssignmentMethod.AUTO_COMPUTED,  # Use existing CHECK constraint value
            business_rule_applied=rule_result.rule_applied.rule_type.value if rule_result.rule_applied else None,
            confidence=rule_result.confidence,
            notes=rule_result.notes,
            assigned_date=datetime.now()
        )
    
    def _run_standard_assignment(self, spot_id: int, spot_data: SpotData) -> AssignmentResult:
    ...
    def _get_overlapping_blocks(self, schedule_id: int, spot_data: SpotData) -> List[Dict[str, Any]]:
        ...
        day_of_week = spot_data.air_date  # Simplified - you'd extract day of week properly
    ...
    def _create_error_result(self, spot_id: int, error_message: str) -> AssignmentResult:
