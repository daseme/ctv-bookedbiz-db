...
class BaseService(ABC):
    ...
    def __init__(self, db_connection: DatabaseConnection):
        ...
        """
        self.db_connection = (
            db_connection  # ✅ Changed from self.db to self.db_connection
        )
    ...
    def safe_transaction(self):
        ...
        logger.debug(
            f"Starting transaction {transaction_id} (depth: {self._transaction_depth})"
        )
        ...
        try:
            ...
            conn.close()
            logger.debug(
                f"Transaction {transaction_id} closed (remaining depth: {self._transaction_depth})"
            )
...
class ExampleService(BaseService):
    ...
    @transaction_required
    def insert_record(
        self, conn: sqlite3.Connection, table_name: str, data: dict
    ) -> int:
    ...
    def conditional_transaction_example(
        self, data: dict, use_transaction: bool
    ) -> bool:
