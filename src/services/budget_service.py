...
class BudgetService:
    ...
    def __init__(
        self, data_path: str, db: Optional[DatabaseConnection] = None
    ):
        """
        Initialize with data directory path and optional DatabaseConnection.

        Args:
            data_path: Path to data directory containing JSON files
            db: Optional DatabaseConnection for advanced features
        """
    ...
    def get_quarterly_budget_summary(
        self, year: int = 2025
        ...
        if year == 2025:
            ...
            for ae_name in budget_2025.keys():
                summary[ae_name] = {}
                for quarter in range(1, 5):
                    summary[ae_name][quarter] = self.get_quarterly_budget(
                        ae_name, quarter
                    )
    ...
    def upload_budget_data(
        ...
        if not self.db:
            raise ValueError("Database connection required for budget uploads")
        ...
        with self.db.transaction() as conn:
            cursor = conn.execute(
                ...
                (version_name, year, created_by, description,
            ...
            for ae_name, monthly_budgets in budget_data.items():
                for month, budget_amount in monthly_budgets.items():
                    ...
                    conn.execute(
                        ...
                        (version_id, ae_name, year, quarter, month,
                         budget_amount)
                        VALUES (?, ?, ?, ?, ?, ?)
                        """,
                        (version_id, ae_name, year, quarter, month,
        ...
        logger.info(
            f"Budget data uploaded successfully: "
            f"version_id={version_id}, year={year}"
        )
    ...
    def _get_company_budget_totals_db(self, year: int) -> Dict[int, float]:
        ...
        try:
            with self.db.connection_ro() as conn:
                cursor = conn.execute(
                    """
                    SELECT quarter,
    ...
    def _get_quarterly_budget_summary_db(
        self, year: int
    ) -> Dict[str, Dict[int, float]]:
        ...
        try:
            ...
            for row in results:
                ...
                budget_summary[ae_name][row["quarter"]] = row["quarter_budget"]

            return budget_summary
        except Exception as e:
            logger.error(
                f"Error getting quarterly budget summary from DB: {e}"
            )
            return {}
    ...
    def get_budget_versions(self, year: int) -> List[Dict[str, Any]]:
        ...
        try:
            with self.db.connection_ro() as conn:
                cursor = conn.execute(
                    """
                    SELECT version_id, version_name, year,
    ...
    def validate_budget_data(self, year: int = 2025) -> Dict[str, Any]:
        ...
        try:
            if year == 2025:
                ...
                total_budget = sum(
                    sum(ae_data.values()) for ae_data in budget_2025.values()
                )
        ...
        try:
            ...
            with self.db.connection_ro() as conn:
                ...
                for row in cursor.fetchall():
                    validation_results["warnings"].append(
                        f"{row['ae_name']} has only "
                        f"{row['month_count']}/12 months"
                    )

                cursor = conn.execute(
                    """
                    SELECT COUNT(DISTINCT ae_name) as ae_count,
