...
class BasicImportService:
    ...
    def import_excel_file(self, 
                         ...
                         validate_spots: bool = True) -> ImportResult:
        """
        ...
        try:
            ...
            logger.info(f"Imported {result.spots_imported} spots, skipped {result.spots_skipped}")
    ...
    def _process_spots(self, spots: List[Spot], result: ImportResult, validate_spots: bool) -> List[Spot]:
        ...
        for i, spot in enumerate(spots):
            try:
                ...
                if agency_name:
                    ...
                    if agency.created_date and agency.created_date.date() == date.today():
                ...
                if not customer:
                    ...
                    if validate_spots:
                        ...
                        if not validation_result.is_valid():
                            logger.warning(f"Customer validation failed for '{customer_name}': {validation_result.errors}")
                ...
                if validate_spots:
                    ...
                    if not validation_result.is_valid():
                        logger.warning(f"Spot validation failed: {validation_result.errors}")
                        result.validation_errors += 1
                ...
                logger.warning(f"Failed to parse bill code '{spot.bill_code}': {str(e)}")
...
def import_excel_to_database(excel_file_path: str, 
                           database_path: str, 
                           limit: Optional[int] = None) -> ImportResult:
    """
