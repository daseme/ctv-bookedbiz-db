...
class BusinessRulesService:
        ...
        return [
            ...
            BusinessRule(
                ...
                min_duration_minutes=config[
                    "nonprofit_campaign_min_duration"
                ],  # 5 hours
                customer_intent=CustomerIntent.INDIFFERENT,
                auto_resolve=True,
                priority=4,
            ...
            BusinessRule(
                ...
                min_duration_minutes=config[
                    "long_duration_content_min_duration"
                ],  # 12 hours
                customer_intent=CustomerIntent.INDIFFERENT,
                auto_resolve=True,
                priority=5,
    ...
    def evaluate_spot(self, spot_data: SpotData) -> BusinessRuleResult:
        ...
        should_exclude, exclusion_reason = self._should_exclude_from_assignment(
            spot_data
        )
        if should_exclude:
    ...
    def _spot_matches_rule(self, spot_data: SpotData, rule: BusinessRule) -> bool:
        ...
        if (
            ...
            and spot_data.duration_minutes < rule.min_duration_minutes
        ):
            return False

        if (
            rule.max_duration_minutes
            and spot_data.duration_minutes > rule.max_duration_minutes
        ):
            return False
    ...
    def _should_exclude_from_assignment(self, spot_data: SpotData) -> tuple[bool, str]:
        ...
        if any(
            keyword in spot_data.bill_code.upper() for keyword in production_keywords
        ):
        ...
        if (
            hasattr(spot_data, "air_date")
            and spot_data.air_date
            and spot_data.air_date > "2025-01-01"
            and spot_data.market_id is None
        ):
        ...
        if (
            ...
            or spot_data.time_in is None
            or spot_data.time_out is None
        ):
    ...
    def remove_rule(self, rule_type: BusinessRuleType):
        ...
        if removed_count > 0:
            self.logger.info(
                f"Removed {removed_count} rule(s) of type {rule_type.value}"
            )
        else:
