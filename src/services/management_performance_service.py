...
class QuarterlyMetrics:
    ...
    def yoy_change_pct(self) -> Optional[float]:
        if self.prior_year_booked is None or self.prior_year_booked == 0:
            return None
        return float(
            (self.booked - self.prior_year_booked) / self.prior_year_booked * 100
        )

    @property
    def customer_yoy_change_pct(self) -> Optional[float]:
        if (
            self.prior_year_customer_count is None
            or self.prior_year_customer_count == 0
        ):
            return None
        return float(
            (self.customer_count - self.prior_year_customer_count)
            / self.prior_year_customer_count
            * 100
        )
...
class ManagementPerformanceService:
    ...
    def _build_metrics_from_cache(
        self, data: Dict[str, Any], year: int, entity_name: Optional[str]
        ...
        for q in range(1, 5):
            if entity_key:
                ...
                q_booked = sum(
                    v
                    for (e, qtr, yr), v in data["booked"].items()
                    if qtr == q and yr == year
                )
                q_budget = sum(v for (e, qtr), v in data["budget"].items() if qtr == q)
                q_forecast = sum(
                    v for (e, qtr), v in data["forecast"].items() if qtr == q
                )
                q_prior = (
                    sum(
                        v
                ...
                q_customers = sum(
                    v
                    for (e, qtr, yr), v in data["customers"].items()
                    if qtr == q and yr == year
                )
                q_prior_customers = (
                    sum(
        ...
        if entity_key:
            ...
            prior_annual_customers = data["annual_customers"].get(
                (entity_key, year - 1)
            )
        else:
            ...
            prior_annual_customers = (
                sum(
                    v
                    for (e, yr), v in data["annual_customers"].items()
                    if yr == year - 1
                )
                or None
            )

        effective_total_forecast = (
            total_forecast if total_forecast > 0 else total_budget
        )
        ...
        annual = AnnualMetrics(
            ...
            pacing_to_forecast=max(
                effective_total_forecast - total_booked, Decimal("0")
            ),
    ...
    def get_management_report(
        self, year: int, pacing_mode: str = "budget"
        ...
        company_quarterly, company_annual = self._build_metrics_from_cache(
            data, year, None
        )
        ...
        entities.sort(
            key=lambda p: p.annual.booked if p.annual else Decimal("0"), reverse=True
        )
