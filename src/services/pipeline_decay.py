...
class PipelineDecayEngine:
    ...
    def apply_pipeline_decay(self, ae_id: str, month: str, revenue_change: float,
                           event_type: DecayEventType, customer: str = None,
                           description: str = None, created_by: str = "system") -> bool:
        """
        ...
        try:
            ...
            new_pipeline = max(0.0, old_pipeline + revenue_change)  # Pipeline can't go negative
            ...
            logger.info(f"Pipeline decay applied: {ae_id} {month} {revenue_change:+.0f} -> {new_pipeline:.0f}")
            return True
    ...
    def _record_decay_event(self, decay_event: DecayEvent):
        ...
        try:
            ...
            month_tracking = decay_data['decay_tracking'][decay_event.ae_id][decay_event.month]
    ...
    def set_calibration_baseline(self, ae_id: str, month: str, pipeline_value: float,
                               calibrated_by: str, session_id: str = None) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Calibration baseline set: {ae_id} {month} -> {pipeline_value:.0f}")
            return True
    ...
    def get_decay_summary(self, ae_id: str, month: str) -> Optional[DecaySummary]:
        ...
        try:
            ...
            decay_data = self._read_decay_data()
            tracking_data = decay_data.get('decay_tracking', {}).get(ae_id, {}).get(month, {})
            
            # Extract values
            current_pipeline = pipeline_data.get('current_pipeline', 0.0)
            calibrated_pipeline = tracking_data.get('calibration_baseline') or current_pipeline
            calibration_date = tracking_data.get('calibration_date') or datetime.utcnow().isoformat() + "Z"
            
            # Calculate days since calibration
            try:
                cal_date = datetime.fromisoformat(calibration_date.replace('Z', '+00:00'))
                days_since_calibration = (datetime.utcnow() - cal_date.replace(tzinfo=None)).days
            except:
    ...
    def get_decay_analytics(self, ae_id: str, months: List[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if all_decay_rates:
                analytics['overall_metrics'] = {
                    ...
                    'avg_decay_percentage': sum(all_decay_percentages) / len(all_decay_percentages),
                    'months_analyzed': len(target_months),
                    'total_decay_events': sum(len(s['decay_events']) for s in monthly_summaries.values())
                }
            
            # Generate trends and recommendations
            analytics['trends'] = self._analyze_decay_trends(monthly_summaries)
            analytics['recommendations'] = self._generate_decay_recommendations(analytics)
    ...
    def _analyze_decay_trends(self, monthly_summaries: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(decay_rates) >= 2:
            ...
            earlier_avg = sum(decay_rates[:-2]) / len(decay_rates[:-2]) if len(decay_rates) > 2 else decay_rates[0]
    ...
    def _generate_decay_recommendations(self, analytics: Dict[str, Any]) -> List[str]:
        ...
        if avg_decay_rate < -100:  # Significant negative decay (pipeline shrinking)
            recommendations.append("Strong revenue performance is reducing pipeline needs faster than expected. Consider lowering future pipeline targets.")
        elif avg_decay_rate > 100:  # Significant positive decay (pipeline growing)
            recommendations.append("Pipeline is growing due to cancellations or removals. Investigate causes and consider mitigation strategies.")
        
        # Recommendation based on trends
        if trends.get('direction') == 'increasing' and avg_decay_rate > 0:
            recommendations.append("Increasing decay trend suggests growing issues with revenue retention. Review recent losses.")
        elif trends.get('direction') == 'decreasing' and avg_decay_rate < 0:
            recommendations.append("Decreasing decay trend indicates improving revenue performance. Consider raising targets.")
        
        # Recommendation based on volatility
        if trends.get('volatility') == 'high':
            recommendations.append("High decay volatility suggests unpredictable revenue patterns. Consider more frequent calibration sessions.")
        
        # Default recommendation if no specific patterns
        if not recommendations:
            recommendations.append("Decay patterns appear normal. Continue regular bi-weekly calibration sessions.")
            ...
            for ae_id, ae_tracking in decay_data.get('decay_tracking', {}).items():
                for month, month_data in ae_tracking.items():
                    ...
                    month_data['decay_events'] = [
                        event for event in month_data.get('decay_events', [])
...
def on_revenue_booked(ae_id: str, month: str, amount: float, customer: str = None,
                     description: str = None, decay_engine: PipelineDecayEngine = None) -> bool:
    """
...
def on_revenue_removed(ae_id: str, month: str, amount: float, customer: str = None,
                      reason: str = None, decay_engine: PipelineDecayEngine = None) -> bool:
    """
