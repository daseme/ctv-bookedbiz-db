...
class DallasGridPopulator(BaseService):
    ...
    def populate_dallas_grid_blocks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.info(f"Dallas Grid population completed successfully in {duration:.2f} seconds")
    ...
    def _validate_day_coverage(self, conn, day_of_week: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(blocks) != 24:
                result['success'] = False
                result['errors'].append(f"{day_of_week}: Expected 24 blocks, found {len(blocks)}")
                return result
            ...
            for time_start, time_end, block_name in blocks:
                ...
                if start_minutes == 0:  # Midnight blocks
                    if end_minutes <= 360:  # 00:00-06:00 range
                        ...
                        result['success'] = False
                        result['errors'].append(f"{day_of_week}: Invalid overnight time range for {block_name}")
