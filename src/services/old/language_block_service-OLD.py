...
class LanguageBlockService:
    ...
    def assign_single_spot(self, spot_id: int) -> AssignmentResult:
        ...
        try:
            ...
            if not spot_data.market_id:
                self.logger.warning(
                    f"Spot {spot_id} has no market_id - cannot assign to language blocks"
                )
                return AssignmentResult(
                    spot_id=spot_id,
                    success=True,
            ...
            self.logger.info(
                f"Successfully assigned spot {spot_id}: {result.customer_intent.value}"
            )
            return result
            ...
            for spot_id in spots_to_process:
                try:
                    ...
                    if self.stats["processed"] % 100 == 0:
                        self.logger.info(
                            f"Processed {self.stats['processed']} spots..."
                        )
    ...
    def _get_applicable_schedule(self, market_id: int, air_date: str) -> Optional[int]:
        ...
        if row:
            self.logger.debug(
                f"Found schedule {row[1]} (ID: {row[0]}) for market {market_id}"
            )
            return row[0]

        # If no result, try fallback approach - just get any active schedule for this market
        self.logger.warning(
            f"No date-matched schedule for market {market_id}, trying fallback"
        )
        ...
        if row:
            self.logger.warning(
                f"Using fallback schedule {row[1]} (ID: {row[0]}) for market {market_id}"
            )
            return row[0]
    ...
    def _get_overlapping_blocks(
        self, schedule_id: int, day_of_week: str, time_in: str, time_out: str
    ) -> List[LanguageBlock]:
        ...
        self.logger.debug(
            f"Found {len(rows)} blocks for schedule {schedule_id}, day {day_of_week}"
        )
    ...
    def _create_assignment(
        ...
        if len(blocks) == 1:
            ...
            return AssignmentResult(
                ...
                requires_attention=intent
                == CustomerIntent.TIME_SPECIFIC,  # Flag mismatches
            )
    ...
    def _save_assignment(self, result: AssignmentResult):
        ...
        if (
            result.customer_intent == CustomerIntent.NO_GRID_COVERAGE
            and not result.schedule_id
        ):
            # For no grid coverage, we'll use a placeholder schedule_id of 0
            # or modify the schema to allow NULL - for now, let's skip these
            self.logger.warning(
                f"Skipping assignment for spot {result.spot_id} - no schedule_id"
            )
            return
        ...
        if result.schedule_id:
            ...
            self.logger.warning(
                f"Cannot save assignment for spot {result.spot_id} - no schedule_id"
            )
