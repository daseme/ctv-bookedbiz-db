...
class MonthClosureService(BaseService):
    ...
    def _get_datetime_values_for_month(self, broadcast_month_display: str) -> List[str]:
        ...
        try:
            ...
            with self.safe_connection() as conn:
                ...
                logger.debug(
                    f"Found {len(values)} values for {broadcast_month_display}"
                )
                return values
    ...
    def close_broadcast_month_with_connection(
        self,
        broadcast_month_display: str,
        closed_by: str,
        ...
        logger.info(
            f"Closing broadcast month within existing transaction: {broadcast_month_display}"
        )

        # Validation (format only, no database checks)
        if not self.parser.validate_broadcast_month_format(broadcast_month_display):
            raise MonthClosureError(
                f"Invalid broadcast month format: '{broadcast_month_display}'"
            )
        ...
        if cursor.fetchone():
            raise MonthClosureError(
                f"Month '{broadcast_month_display}' is already closed"
            )
        ...
        if spot_count == 0:
            raise MonthClosureError(
                f"Cannot close '{broadcast_month_display}': No data exists for this month"
            )
        ...
        logger.info(
            f"Closed '{broadcast_month_display}' within transaction: {updated_count} spots marked historical"
        )
        return True
    ...
    def _get_display_format_for_datetime(self, datetime_value: str) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Could not convert datetime '{datetime_value}' to display format"
            )
            return datetime_value  # Return as-is if conversion fails

    def close_broadcast_month(
        self, broadcast_month_display: str, closed_by: str, notes: str = None
    ) -> bool:
        ...
        if not self.parser.validate_broadcast_month_format(broadcast_month_display):
            raise MonthClosureError(
                f"Invalid broadcast month format: '{broadcast_month_display}'. Expected format: 'Mmm-YY'"
            )
        ...
        if not datetime_values:
            raise MonthClosureError(
                f"Cannot close '{broadcast_month_display}': No data exists for this month. Import data first, then close the month."
            )
        ...
        try:
            with self.safe_transaction() as conn:
                ...
                logger.info(
                    f"Successfully closed '{broadcast_month_display}': {updated_count} spots marked as historical from {len(datetime_values)} datetime values"
                )
                return True
    ...
    def is_month_closed(self, broadcast_month_display: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error checking if month '{broadcast_month_display}' is closed: {e}"
            )
            return False
    ...
    def get_closed_months(self, broadcast_months_display: List[str]) -> List[str]:
        ...
        try:
            with self.safe_connection() as conn:
                ...
                logger.debug(
                    f"Found {len(closed_months)} closed months out of {len(broadcast_months_display)} checked"
                )
    ...
    def validate_months_for_import(
        self, broadcast_months_display: List[str], import_mode: str
    ) -> ValidationResult:
        """
        ...
        """
        logger.info(
            f"Validating {len(broadcast_months_display)} months for {import_mode} import"
        )

        # Get closed and open months
        closed_months = self.get_closed_months(broadcast_months_display)
        open_months = [
            month for month in broadcast_months_display if month not in closed_months
        ]
        ...
        if import_mode == "WEEKLY_UPDATE":
            ...
            if closed_months:
                ...
                suggested_action = f"Remove rows with broadcast_month in {closed_months} from Excel file and retry weekly update."
            ...
            if closed_months:
                logger.warning(
                    f"Historical import includes already closed months: {closed_months}"
                )
    ...
    def bulk_close_months(
        self, broadcast_months: List[str], closed_by: str, notes: str = None
    ) -> dict:
        """
        ...
        results = {"success": [], "failed": [], "already_closed": []}
        ...
        logger.info(
            f"Bulk closure completed: {len(results['success'])} success, {len(results['failed'])} failed, {len(results['already_closed'])} already closed"
        )
        return results
    ...
    def get_month_statistics(self, broadcast_month_display: str) -> dict:
        ...
        try:
            with self.safe_connection() as conn:
                ...
                if total_spots == 0:
                    return {
                        ...
                        "closure_info": self.get_month_closure_info(
                            broadcast_month_display
                        ),
                    }
            ...
            logger.error(
                f"Error getting statistics for '{broadcast_month_display}': {e}"
            )
            return {"broadcast_month": broadcast_month_display, "error": str(e)}


# Convenience functions for simple usage
def close_month(
    db_path: str, broadcast_month: str, closed_by: str, notes: str = None
) -> bool:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--list-closed", action="store_true", help="List all closed months"
    )
    parser.add_argument(
        "--test-validation", action="store_true", help="Test validation logic"
    )
    ...
    try:
        if args.close_month:
            ...
            try:
                result = service.close_broadcast_month(
                    args.close_month, args.closed_by, args.notes
                )
                if result:
            ...
            if closed_months:
                ...
                for info in closed_months:
                    print(
                        f"  {info['broadcast_month']} - Closed {info['closed_date']} by {info['closed_by']}"
                    )
                    if info["notes"]:
                        print(f"    Notes: {info['notes']}")
            else:
