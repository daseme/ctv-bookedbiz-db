...
class BatchEntityResolver:
    ...
    def build_entity_cache_from_excel(self, excel_file: str) -> None:
        ...
        logger.info(
            "Performance improvements still active through cached lookups during import"
        )
        ...
        try:
            ...
            for bill_code in bill_codes:
                try:
                    ...
                    agency_id = (
                        self._lookup_agency_direct(agency_part, conn)
                        if agency_part
                        else None
                    )
    ...
    def _individual_entity_lookup(self, bill_code: str, conn) -> EntityLookupResult:
        ...
        try:
            if ":" in bill_code:
                ...
                result.customer_id = self._lookup_customer_quick(
                    customer_part.strip(), conn
                )
                result.agency_id = self._lookup_agency_quick(agency_part.strip(), conn)
            else:
                result.customer_id = self._lookup_customer_quick(
                    bill_code.strip(), conn
                )
    ...
    def get_performance_stats(self) -> Dict[str, int]:
        ...
        cache_hit_rate = (
            (self.cache_stats["cache_hits"] / total_lookups * 100)
            if total_lookups > 0
            else 0
        )
