    ...
    def _is_historical_month(self, broadcast_month_str: str) -> bool:
        ...
        try:
            ...
            broadcast_date = datetime.strptime(
                broadcast_month_str.split(" ")[0], "%Y-%m-%d"
            )
    ...
    def get_customer_deals(self, ae_name: str, month: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                is_past = (
                    self._is_historical_month(row_dict["broadcast_month"])
                    or row_dict["is_historical"]
                )
            ...
            logger.info(
                f"Found {len(results)} deals for {ae_name} in {month} (pattern: {month_pattern})"
            )
            return results
    ...
    def categorize_deals(self, deals: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        def aggregate_by_customer(deal_list):
            ...
            for deal in deal_list:
                ...
                customer_aggregates[customer_name]["total_revenue"] += (
                ...
                if deal["expected_close_date"]:
                    if (
                        customer_aggregates[customer_name]["first_spot"] is None
                        or deal["expected_close_date"]
                        < customer_aggregates[customer_name]["first_spot"]
                    ):
                        customer_aggregates[customer_name]["first_spot"] = deal[
                            "expected_close_date"
                        ]
