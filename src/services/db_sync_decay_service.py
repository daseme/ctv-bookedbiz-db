...
class DatabaseSyncDecayService:
    ...
    def __init__(self, ae_service, pipeline_service):
        ...
        self.last_sync_file = pipeline_service.data_path + '/last_sync_state.json' if pipeline_service.data_path else None
    ...
    def sync_and_apply_decay(self, ae_id: str = None) -> Dict[str, Any]:
        ...
        try:
                ...
                try:
                    ...
                    for change in ae_changes:
                        if self._apply_decay_for_change(change):
                            ...
                            sync_results['errors'].append(f"Failed to apply decay for {change['ae_name']} {change['month']}")
            ...
            logger.info(f"Sync complete: {sync_results['aes_synced']} AEs, {sync_results['decay_adjustments_applied']} decay adjustments")
    ...
    def _sync_ae_revenue_data(self, ae: Dict[str, Any], last_state: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
            ...
            try:
                ...
                if abs(revenue_change) >= 1000:  # $1000 threshold
                    change = {
                        ...
                        'change_type': 'revenue_increase' if revenue_change > 0 else 'revenue_decrease',
                        'detected_at': datetime.utcnow().isoformat() + "Z",
                        'source': 'database_sync'
                    }
                    changes.append(change)
                    
                    logger.info(f"Revenue change detected: {ae_name} {month} {last_revenue} -> {current_revenue} ({revenue_change:+.0f})")
            ...
            for ae in ae_list:
                ...
                for month in months:
                    try:
                        ...
                        logger.warning(f"Could not get revenue for {ae_name} {month}: {e}")
                ...
                if os.path.exists(self.last_sync_file):
                    ...
                    with open(self.last_sync_file, 'r') as f:
                        ...
                        if status['last_sync_timestamp']:
                            last_sync = datetime.fromisoformat(status['last_sync_timestamp'].replace('Z', '+00:00'))
                            time_since = datetime.utcnow() - last_sync.replace(tzinfo=None)
                            status['hours_since_last_sync'] = time_since.total_seconds() / 3600
...
class AutoSyncAEService:
    ...
    def __init__(self, original_ae_service, pipeline_service):
        ...
        self.pipeline_service = pipeline_service
        self.decay_sync = DatabaseSyncDecayService(original_ae_service, pipeline_service)
    ...
    def get_ae_by_id(self, ae_id: str, auto_sync: bool = True):
        ...
        if auto_sync:
            try:
                ...
                if sync_results['decay_adjustments_applied'] > 0:
                    logger.info(f"Auto-sync applied {sync_results['decay_adjustments_applied']} decay adjustments for {ae_id}")
            except Exception as e:
        ...
        if ae_data and hasattr(self.pipeline_service, 'decay_engine'):
            try:
                ...
                recent_activity = decay_analytics.get('overall_metrics', {}).get('total_decay_events', 0)
    ...
    def get_filtered_ae_list(self, auto_sync: bool = False):
        ...
        if auto_sync:
            try:
                ...
                if sync_results['decay_adjustments_applied'] > 0:
                    logger.info(f"Auto-sync applied {sync_results['decay_adjustments_applied']} total decay adjustments")
            except Exception as e:
        ...
        for ae in ae_list:
            if hasattr(self.pipeline_service, 'decay_engine'):
                try:
                    ...
                    decay_summary = self.pipeline_service.get_pipeline_decay_summary(ae['ae_id'], datetime.now().strftime('%Y-%m'))
                    ae['has_decay_activity'] = bool(decay_summary and decay_summary.get('decay_events'))
...
def simple_ae_decay_check(ae_service, pipeline_service, ae_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not hasattr(pipeline_service, 'decay_engine') or not pipeline_service.decay_engine:
            logger.info("Decay system not available - pipeline service has no decay engine")
            return {
                'success': True,
                'decay_applied': 0,
                'changes_found': 0,
                ...
                class AEServiceWrapper:
                    ...
                    def get_filtered_ae_list(self):
                        ...
                        return [
                            ...
                            {'ae_id': 'ae_riley_van_patten', 'name': 'Riley Van Patten'},
                            {'ae_id': 'ae_white_horse_international', 'name': 'White Horse International'}
                        ]
                    ...
                    def get_monthly_revenue(self, ae_name, month):
                        ...
                        try:
                            ...
                            if monthly_data and hasattr(monthly_data, 'to_dict'):
                                revenue_data = monthly_data.to_dict().get('revenue_data', [])
                                total_revenue = 0
                            ...
                            logger.warning(f"Could not get monthly revenue for {ae_name} {month}: {e}")
                        
                        return 0
                
                ae_service_wrapper = AEServiceWrapper(ae_service)
                _global_decay_service = DatabaseSyncDecayService(ae_service_wrapper, pipeline_service)
...
def get_ae_with_decay_info(ae_service, pipeline_service, ae_id: str) -> Optional[Dict[str, Any]]:
    """
...
def enhance_monthly_summary_with_decay(monthly_summary: List[Dict], ae_id: str, pipeline_service) -> List[Dict]:
    """
    ...
    try:
        for month_data in monthly_summary:
            ...
            try:
                ...
                if decay_summary:
                    month_data.update({
                        'has_decay_activity': len(decay_summary.get('decay_events', [])) > 1,
                        'total_decay': decay_summary.get('total_decay', 0),
                        'days_since_calibration': decay_summary.get('days_since_calibration', 0),
                        'decay_events_count': len(decay_summary.get('decay_events', [])),
                        'calibrated_pipeline': decay_summary.get('calibrated_pipeline'),
