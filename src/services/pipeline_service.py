...
class PipelineService:
    ...
    def __init__(self, db_connection=None, data_path: str = None, 
        ...
        # File paths
        self.pipeline_file = os.path.join(data_path, 'pipeline_data.json') if data_path else None
        self.sessions_file = os.path.join(data_path, 'review_sessions.json') if data_path else None
    ...
    def _ensure_data_integrity(self):
        ...
        if self.data_source in [DataSourceType.JSON_PRIMARY, DataSourceType.DB_PRIMARY]:
            ...
            if not consistency_result.is_consistent:
                logger.warning(f"Data consistency issues detected: {len(consistency_result.conflicts)} conflicts")
        ...
        if not os.path.exists(self.pipeline_file):
            default_pipeline = {
                ...
                "metadata": {
                    ...
                    "features": ["decay_tracking", "real_time_adjustments", "calibration_baselines"]
                }
            }
    ...
    def apply_revenue_booking(self, ae_id: str, month: str, amount: float,
                            customer: str = None, description: str = None) -> bool:
        """
    ...
    def apply_revenue_removal(self, ae_id: str, month: str, amount: float,
                            customer: str = None, reason: str = None) -> bool:
        """
    ...
    def set_pipeline_calibration(self, ae_id: str, month: str, pipeline_value: float,
    ...
    def get_pipeline_decay_summary(self, ae_id: str, month: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_pipeline_data_with_decay(self, ae_id: str, month: str = None) -> Dict[str, Any]:
        """
            ...
            if self.decay_engine:
                ...
                if decay_summary:
                    summary.update({
                        ...
                        'decay_rate_per_day': decay_summary.get('decay_rate_per_day', 0),
                        'decay_percentage': decay_summary.get('decay_percentage', 0),
                        'decay_events_count': len(decay_summary.get('decay_events', [])),
                        'has_decay_activity': len(decay_summary.get('decay_events', [])) > 1  # More than just calibration
                    })
    ...
    def update_pipeline_data(self, ae_id: str, month: str, pipeline_update: Dict[str, Any], 
                           updated_by: str, review_session_id: str = None) -> bool:
        """
        ...
        try:
            ...
            is_calibration = (
                'current_pipeline' in pipeline_update and
                updated_by not in ['decay_engine', 'system_decay'] and
                not updated_by.startswith('decay_engine_')
            )
            
            # Perform the base update
            success = self._update_pipeline_data_base(ae_id, month, pipeline_update, updated_by, review_session_id)
    ...
    def _update_pipeline_data_base(self, ae_id: str, month: str, pipeline_update: Dict[str, Any], 
                                 updated_by: str, review_session_id: str = None) -> bool:
        """Base pipeline update without decay handling."""
        if self.data_source in [DataSourceType.JSON_ONLY, DataSourceType.JSON_PRIMARY]:
            return self._update_pipeline_data_json(ae_id, month, pipeline_update, updated_by, review_session_id)
        else:
            return self._update_pipeline_data_db(ae_id, month, pipeline_update, updated_by, review_session_id)
    
    def _update_pipeline_data_json(self, ae_id: str, month: str, pipeline_update: Dict[str, Any], 
                                 updated_by: str, review_session_id: str) -> bool:
    ...
    def get_data_source_info(self) -> Dict[str, Any]:
        ...
        base_info = {
            'data_source': self.data_source.value,
            'json_file_exists': os.path.exists(self.pipeline_file) if self.pipeline_file else False,
