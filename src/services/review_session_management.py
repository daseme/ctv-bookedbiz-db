...
class ReviewSession:
    ...
    def completed_aes(self) -> int:
        ...
        return sum(
            1
            for review in self.ae_reviews.values()
...
class ReviewSessionManager:
    ...
    def create_review_session(
        self, session_name: str, created_by: str, target_completion_date: str = None
    ) -> str:
        """
        ...
        for ae in ae_list:
            ae_reviews[ae["ae_id"]] = AEReviewSession(
                ae_id=ae["ae_id"], ae_name=ae["name"], status=AEReviewStatus.PENDING
            )
    ...
    def complete_review_session(
        self, session_id: str, completed_by: str, session_notes: str = ""
    ) -> bool:
    ...
    def apply_calibration(
        ...
        try:
            ...
            old_pipeline = (
                current_data.get("current_pipeline", 0) if current_data else 0
            )
            ...
            if session_id in sessions_data["sessions"]:
                ...
                if ae_id in session_data["ae_reviews"]:
                    ...
                    session_data["ae_reviews"][ae_id]["calibrations"].append(
                        asdict(calibration)
                    )
            ...
            logger.info(
                f"Applied calibration: {ae_id} {month} {old_pipeline} -> {new_pipeline}"
            )
            return True
    ...
    def bulk_calibration(
        self, session_id: str, calibrations: List[Dict[str, Any]], calibrated_by: str
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Bulk calibration: {results['successful']} successful, {results['failed']} failed"
        )
        return results
    ...
    def get_session_status(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            total_calibrations = sum(
                len(ae.get("calibrations", [])) for ae in ae_reviews.values()
            )
            ...
            status = {
                ...
                "progress": {
                    ...
                    "completion_percentage": (completed_aes / total_aes * 100)
                    if total_aes > 0
                    else 0,
                ...
                "timing": {
                    ...
                    "target_completion_date": session_data.get(
    ...
    def get_next_review_recommendation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            last_completed = datetime.fromisoformat(
                last_session["completed_at"].replace("Z", "+00:00")
            )
            days_since_last = (
                datetime.utcnow() - last_completed.replace(tzinfo=None)
            ).days
            ...
            if days_since_last >= 14:
                ...
                next_review_date = last_completed.replace(tzinfo=None) + timedelta(
                    days=14
                )
                return {
