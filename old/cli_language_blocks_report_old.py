...
def generate_enhanced_language_block_report():
    ...
    try:
        ...
        print(f"{'Category':25} {'Spots':>10} {'Revenue':>15} {'Avg Rev':>10} {'Purpose':>20}")
        print("-" * 90)
        
        if worldlink and worldlink['spots']:
            print(f"{'WorldLink Infomercials':25} {worldlink['spots']:>10,} ${worldlink['revenue']:>14,.0f} ${worldlink['avg_revenue']:>9,.0f} {'Broad Reach DR':>20}")
        
        if gov_roadblock and gov_roadblock['spots']:
            print(f"{'Government Roadblocks':25} {gov_roadblock['spots']:>10,} ${gov_roadblock['revenue']:>14,.0f} ${gov_roadblock['avg_revenue']:>9,.0f} {'All Communities':>20}")
        
        if political and political['spots']:
            print(f"{'Political Campaigns':25} {political['spots']:>10,} ${political['revenue']:>14,.0f} ${political['avg_revenue']:>9,.0f} {'Broad Political':>20}")
        
        if language_targeted and language_targeted['spots']:
            print(f"{'Language-Targeted':25} {language_targeted['spots']:>10,} ${language_targeted['revenue']:>14,.0f} ${language_targeted['avg_revenue']:>9,.0f} {'Specific Language':>20}")
        
        if unassigned and unassigned['spots']:
            print(f"{'Unassigned/Grid Gaps':25} {unassigned['spots']:>10,} ${unassigned['revenue']:>14,.0f} ${unassigned['avg_revenue']:>9,.0f} {'Needs Assignment':>20}")
        ...
        if worldlink and worldlink['spots']:
            ...
            print(f"{'WorldLink Infomercials':25} {spots_pct:>9.1f}% {revenue_pct:>14.1f}%")
        
        if gov_roadblock and gov_roadblock['spots']:
            spots_pct = (gov_roadblock['spots'] / total_spots) * 100
            revenue_pct = (gov_roadblock['revenue'] / total_revenue) * 100
            print(f"{'Government Roadblocks':25} {spots_pct:>9.1f}% {revenue_pct:>14.1f}%")
        
        if political and political['spots']:
            spots_pct = (political['spots'] / total_spots) * 100
            revenue_pct = (political['revenue'] / total_revenue) * 100
            print(f"{'Political Campaigns':25} {spots_pct:>9.1f}% {revenue_pct:>14.1f}%")
        ...
        if unassigned and unassigned['spots']:
            ...
            print(f"{'Unassigned/Grid Gaps':25} {spots_pct:>9.1f}% {revenue_pct:>14.1f}%")
        ...
        for case in edge_cases:
            print(f"{case['status']:25} {case['spots']:>10,} ${case['revenue']:>14,.0f}")
        ...
        if lang_performance:
            print(f"{'Language':15} {'Spots':>8} {'Revenue':>12} {'Avg Rev':>10} {'Customers':>10}")
            print("-" * 70)
            for lang in lang_performance:
                print(f"{lang['language_name']:15} {lang['language_targeted_spots']:>8,} ${lang['language_targeted_revenue']:>11,.0f} ${lang['avg_revenue']:>9,.0f} {lang['unique_customers']:>10}")
        ...
        if grid_coverage:
            ...
            for category in grid_coverage:
                print(f"{category['time_category']:30} {category['spots']:>8,} ${category['revenue']:>11,.0f}")
        ...
        if targeted_customers:
            ...
            for customer in targeted_customers:
                print(f"{customer['customer_name'][:30]:30} {customer['spots']:>8,} ${customer['revenue']:>11,.0f} {customer['languages_used']:>10}")
        ...
        if real_penetration:
            assignment_rate = (real_penetration['assigned_spots'] / real_penetration['total_spots']) * 100
            revenue_rate = (real_penetration['assigned_revenue'] / real_penetration['total_revenue']) * 100
            unassigned_revenue = real_penetration['total_revenue'] - real_penetration['assigned_revenue']
            ...
            print(f"   • Growth Opportunity: ${unassigned_revenue:,.0f} in unassigned revenue")
            print()
        
        print("📈 BUSINESS INTELLIGENCE SUMMARY:")
        print("   • Broad-reach campaigns (infomercials, roadblocks, political) work by design")
