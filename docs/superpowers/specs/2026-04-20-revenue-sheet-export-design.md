...
- **Per-contract rate semantics — deferred.** The "never average rates across contracts" concern was considered and deferred to v2. Typical rate variation (e.g., `Internal Ad Sales` 15% vs. `Branded Content` 0% for the same customer) is already handled correctly because `revenue_class` is a row identity field, so those rates naturally land on different rows. If real data ever shows rate drift within a single `(customer, revenue_class)` tuple over time, revisit: add `contract` to the API `GROUP BY` and switch PQ from "SUM across months" to "current-contract representative row" for percent columns.
