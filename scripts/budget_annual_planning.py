...
class AEPerformance:
    ...
    def top_customers(self, limit: int = 10) -> List[Customer]:
        ...
        return sorted(self.customers, key=lambda c: c.revenue_2025, reverse=True)[
            :limit
        ]
...
class RevenueRepository:
    ...
    def get_ae_new_vs_returning_customers(
        self, ae_name: str
        ...
        with sqlite3.connect(self._db_path) as conn:
            ...
            logger.info(
                f"   📊 Found {len(customers_2024)} unique normalized customers in 2024"
            )
            logger.info(
                f"   📊 Found {len(customers_2025)} unique normalized customers in 2025"
            )
            ...
            for normalized_key, customer in customers_2025.items():
                if normalized_key in customers_2024:
                    ...
                    logger.debug(
                        f"RETURNING: '{customer.name}' -> normalized: '{normalized_key}'"
                    )
                else:
                    new_customers.append(customer)
                    logger.debug(
                        f"NEW: '{customer.name}' -> normalized: '{normalized_key}'"
                    )
            ...
            logger.info(
                f"   👥 Customer analysis: {len(new_customers)} new, {len(returning_customers)} returning"
            )

            # Additional debug: Show which customers are being classified where
            if new_customers:
                logger.info(
                    f"   🆕 Top new customers: {[c.name for c in new_customers[:3]]}"
                )
            if returning_customers:
                logger.info(
                    f"   🔄 Top returning customers: {[c.name for c in returning_customers[:3]]}"
                )

            return new_customers, returning_customers

    def get_ae_sector_trends(
        self, ae_name: str, years: List[int]
    ) -> List[SectorSummary]:
        """Get sector performance trends for multiple years"""
        year_conditions = " OR ".join(
            [f"s.broadcast_month LIKE '%-{str(year)[-2:]}'" for year in years]
        )
        ...
        with sqlite3.connect(self._db_path) as conn:
            ...
            logger.info(
                f"   📅 Years found in data: {sorted(years_found)} (requested: {years})"
            )
...
class TailscaleFileTransfer:
    ...
    def copy_file_to_desktop(self, local_file_path: str) -> bool:
        ...
        try:
            ...
            logger.error("   ❌ Transfer timed out after 30 seconds")
            return False
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(
                    f"✅ Desktop {self._desktop_name} is reachable via Tailscale"
                )
                return True
            else:
                logger.warning(
                    f"⚠️  Desktop {self._desktop_name} not reachable: {result.stderr}"
                )
                return False
...
class ExcelReportGenerator:
    ...
    def create_ae_worksheet(
        ...
        with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
            ...
            self._create_new_vs_returning_sheet(
                writer, new_customers, returning_customers
            )
    ...
    def _format_currency_columns(self, worksheet) -> None:
        ...
        for col_idx, cell in enumerate(header_row, 1):
            if cell.value and any(
                keyword in str(cell.value).lower() for keyword in currency_keywords
            ):
            ...
            if any(cell.value and "TOTAL" in str(cell.value).upper() for cell in row):
                for cell in row:
                    if cell.value is not None:
                        ...
                        if (
                            hasattr(current_style, "number_format")
                            and "$" in current_style.number_format
                        ):
                            cell.number_format = "$#,##0.00"

    def _create_customer_detail_sheet(
        self, writer: pd.ExcelWriter, ae_performance: AEPerformance
    ) -> None:
        ...
        for customer in sorted(
            ae_performance.customers, key=lambda c: c.revenue_2025, reverse=True
        ):
            customer_data.append(
                {
                    ...
                    ),  # Numeric for Excel formatting
                    "Spots": customer.spot_count,
                    "Avg per Spot": float(
                        customer.revenue_2025 / max(customer.spot_count, 1)
                    ),  # Numeric for Excel formatting
                }
            )
    ...
    def _create_raw_data_sheet(
        self, writer: pd.ExcelWriter, raw_data: List[Dict], ae_name: str
    ) -> None:
        """Create raw data sheet matching RevenueDB format with numeric values and totals"""
        if not raw_data:
            # Create empty sheet with message
            empty_df = pd.DataFrame(
                {"Message": [f"No raw data available for {ae_name}"]}
            )
        ...
        months = (
            sorted(raw_df["broadcast_month"].dropna().unique())
            if "broadcast_month" in raw_df.columns
            else []
        )
        ...
        if months:
            ...
            pivot_df = pivot_df.rename(
                columns={"ae_name": "AE", "sector": "Sector", "customer": "Customer"}
            )
            ...
            month_columns = [
                ...
                if col not in ["AE", "Sector", "Customer"]
            ]
            ...
            final_columns = (
                ["AE", "Sector", "Customer"]
                + sorted_month_columns
                + quarter_columns
                + ["Total"]
            )
            pivot_df = pivot_df[
                [col for col in final_columns if col in pivot_df.columns]
            ]
            ...
            totals_row = {"AE": "", "Sector": "", "Customer": "TOTAL"}
            ...
            logger.info(
                f"   📊 Raw Data 2025 sheet: {len(pivot_df) - 1} customer records with {len(month_columns)} months + totals"
            )
    ...
    def _create_monthly_breakdown_sheet(
        self, writer: pd.ExcelWriter, monthly: List[MonthlyRevenue]
    ) -> None:
    ...
    def _create_sector_trends_sheet(
        self, writer: pd.ExcelWriter, sector_analysis: List[SectorSummary]
    ) -> None:
        ...
        for year in years:
            year_total = sum(
                s.revenue_by_year.get(year, Decimal("0")) for s in sector_analysis
            )
    ...
    def _create_language_sheet(
        self, writer: pd.ExcelWriter, language_analysis: List[LanguageRevenue]
    ) -> None:
        """Create language revenue analysis sheet with numeric values and totals"""
        if not language_analysis:
            # Create empty sheet with message
            empty_df = pd.DataFrame(
                {"Message": ["No language data available for analysis"]}
            )
        ...
        language_data.append(
            {
                ...
                "Avg per Spot": float(
                    total_revenue / max(total_spots, 1)
                ),  # Numeric for Excel formatting
                "Avg per Customer": float(
                    total_revenue / max(total_customers, 1)
                ),  # Numeric for Excel formatting
            }
        )
        ...
        logger.info(
            f"   📊 Language Analysis sheet: {len(language_analysis)} languages analyzed"
        )

    def _create_new_vs_returning_sheet(
        self,
        ...
        returning_customer_pct = (
            len(returning_customers) / max(total_customers, 1)
        ) * 100
        ...
        summary_data = [
            ...
            [
                "New Customers (2025)",
                len(new_customers),
                f"{new_customer_pct:.1f}%",
                float(new_revenue),
                f"{new_revenue_pct:.1f}%",
                float(new_revenue / max(len(new_customers), 1)),
            ],
            [
                "Returning Customers",
                len(returning_customers),
                f"{returning_customer_pct:.1f}%",
                float(returning_revenue),
                f"{returning_revenue_pct:.1f}%",
        ...
        summary_data.extend(
            [
                ...
                [
                    ...
                    float(
                        (new_total_revenue + returning_total_revenue)
                        / max(new_total_spots + returning_total_spots, 1)
                    ),
                    "",
                ],
            ]
        ...
        analysis_df.to_excel(
            writer, sheet_name="New vs Returning", index=False, header=False
        )

        logger.info(
            f"   👥 New vs Returning sheet: {len(new_customers)} new, {len(returning_customers)} returning customers"
        )

    def create_summary_report(
        self, ae_performances: List[AEPerformance], output_dir: Path
    ) -> str:
        ...
        with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
            ...
            company_data = [
                ...
                [
                    "Average per AE:",
                    f"${total_revenue / max(len(ae_performances), 1):,.2f}",
                    "",
                    "",
                ],
                ["", "", "", ""],
            ...
            company_df = pd.DataFrame(
                company_data, columns=["Metric", "Value", "Extra1", "Extra2", "Extra3"]
            )
            company_df.to_excel(
                writer, sheet_name="Company Summary", index=False, header=False
            )
...
def main() -> int:
    ...
    try:
        logger.info(
            "🚀 Starting Database-Driven AE Budget Sheet Generation for 2026 Planning"
        )
        logger.info("=" * 70)
        ...
        if not file_transfer.test_connection():
            logger.warning(
                "⚠️  Desktop not reachable - files will be generated locally only"
            )
        ...
        logger.info(
            f"📊 Database contains: {db_info['spots_2025']:,} spots in 2025 from {db_info['total_aes']} AEs"
        )
        ...
        for ae_name in available_aes:
            ...
            if ae_performance.total_revenue > 0:
                # Get supporting data
                sector_analysis = revenue_service.get_sector_analysis(
                    ae_name, [2021, 2022, 2023, 2024, 2025]
                )
                ...
                new_customers, returning_customers = (
                    revenue_service.get_new_vs_returning_customers(ae_name)
                )
                ...
                logger.info(
                    f"   ✅ {ae_name}: ${ae_performance.total_revenue:,.2f} from {ae_performance.customer_count} customers ({ae_performance.spot_count} spots)"
                )
            else:
        ...
        if ae_performances:
            summary_file = report_generator.create_summary_report(
                ae_performances, output_dir
            )
        ...
        if transfer_enabled and generated_files:
            ...
            success_count, total_count = file_transfer.copy_directory_contents(
                output_dir
            )

            if success_count == total_count:
                logger.info(
                    f"✅ All {total_count} files successfully transferred to desktop!"
                )
            elif success_count > 0:
                logger.warning(
                    f"⚠️  {success_count}/{total_count} files transferred successfully"
                )
            else:
                logger.error("❌ Failed to transfer any files to desktop")
        ...
        if transfer_enabled:
            logger.info("📤 Files transferred to: desktop-7402tkp")
